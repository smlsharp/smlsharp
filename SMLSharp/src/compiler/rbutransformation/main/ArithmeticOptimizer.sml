 ...
 * @version $Id: ArithmeticOptimizer.sml,v 1.3 2008/01/10 04:43:12 katsu Exp $
 *)
...
structure ArithmeticOptimizer : ARITHMETICOPTIMIZER = struct
...
(*Printing utilities*)
  ...
  fun formatExp e =
      ...
      | SUB(e1,e2) => (formatExp e1) ^ " - " ^ (formatExp e2)
      | NOT e => "NOT(" ^ formatExp e ^ ")"
...
(**************************************************)
  ...
  fun eq (CONST c1, CONST c2) = c1 = c2
    ...
    | eq (RSHIFT (exp11,exp12), RSHIFT(exp21,exp22)) =
      eq(exp11,exp21) andalso eq(exp12,exp22)
    | eq (SUB (exp11,exp12), SUB(exp21,exp22)) =
      eq(exp11,exp21) andalso eq(exp12,exp22)
    | eq (NOT exp1, NOT exp2) = eq(exp1, exp2)
    | eq (_,_) = false
  ...
  fun insert (batch,exp) =
      ...
      | SUB(e1,e2) => insertSub(batch,e1,e2)
      | NOT e => insertNot(batch,e)
  ...
  (*insert an SUB formula and performing optimization
   * e - 0 = e
   * (e - c1) - c2 = e - (c1 + c2)
   * c1 - (e - c2) = (c1 + c2) - e
   *)
  and insertSub (batch,e1,e2) =
      let
        val (batch,v1) = insert(batch,e1)
        val (batch,v2) = insert(batch,e2)
      in
        case (lookup(batch,v1),lookup(batch,v2)) of
          (CONST c1,CONST c2) => insertAtom(batch,CONST (c1-c2))
        | (e,CONST 0w0) => (batch,v1)
        | (SUB(e,CONST c1),CONST c2) => insertSub(batch,e,CONST(c1+c2))
        | (CONST c1,SUB(e,CONST c2)) => insertSub(batch,CONST(c1+c2),e)
        | (e1',e2') => 
          insertBinaryAtom(batch,SUB,atomArg(v1,e1'),atomArg(v2,e2'))
      end

  (*insert an NOT formula and performing optimization *)
  and insertNot (batch,e1) =
      let
        val (batch,v1) = insert(batch,e1)
      in
        case lookup(batch,v1) of
          CONST c => insertAtom(batch,CONST(Word32.notb c))
        | e1' => insertAtom(batch,NOT(atomArg(v1,e1')))
      end
  ...
  fun dependences (batch, e) =
      ...
      | SUB(e1,e2) => VSet.union(dependences(batch,e1),dependences(batch,e2))
      | NOT(e1) => dependences(batch,e1)
