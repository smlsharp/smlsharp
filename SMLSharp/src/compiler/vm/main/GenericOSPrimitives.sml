 ...
 * @version $Id: GenericOSPrimitives.sml,v 1.15 2007/12/04 06:35:47 kiyoshiy Exp $
 *)
...
*)
  ...
  fun GenericOS_fileReadBuf
      ...
      in
        Word8ArraySlice.copyVec
            {
              src = Word8VectorSlice.slice (byteVector,0,SOME(byteVectorLength)),
              (*si = 0,*)
              dst = byteArray,
  ...
  fun GenericOS_fileWrite 
      ...
      let
        ...
        val string =
            Byte.unpackString
                (Word8ArraySlice.slice(byteArray, SInt32ToInt start, SOME(SInt32ToInt bytes)))
      in
        ...
        then #print (VM.getStandardOutput VM) string
        else
          if STDERR_FILENO = fileNo
          then #print (VM.getStandardError VM) string
          else
            case getStreamOfFileNo fileNo of
        ...
        val timeOutOpt =
            ...
            | SOME block =>
              case SLD.valueToTupleElements heap block of
                [Int sec, Int usec] => SOME(Time.fromSeconds (SInt32.toLarge sec))
  ...
  fun GenericOS_getFileModTime VM heap [nameAddress as Pointer _] =
      ...
      in [Int(SInt32.fromLarge seconds)]
      end
  ...
  fun GenericOS_setFileTime VM heap [nameAddress as Pointer _, Int seconds] =
      let
        val name = SLD.valueToString heap nameAddress
        val time = Time.fromSeconds (SInt32.toLarge seconds)
