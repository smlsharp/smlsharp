 ...
 * @version $Id: PathResolver.sml,v 1.1 2008/11/19 20:15:45 ohori Exp $
 *)
...
struct
  ...
  structure PE = PathResolverError

  exception InvalidPath = PE.InvalidPath
        ...
        val getVariable' =
            fn name =>
               case getVariable name of
                 SOME value => value
               | NONE => raise PE.InvalidPath ("undefined variable: " ^ name)
        fun substPath path = 
            case StringCvt.scanString (scan getVariable') path
             of NONE =>
                raise PE.InvalidPath ("invalid path format: " ^ path)
              | SOME path => path
           ...
           fn path =>
              ...
              in
                ...
                handle OS.SysErr (message, err) =>
                       raise PE.InvalidPath ("file not found: " ^ parsedPath)
              end
      end
  end
