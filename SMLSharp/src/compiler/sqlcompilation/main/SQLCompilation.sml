...
struct
  ...
  fun IntConst (const, loc) =
      (R.RCCONSTANT {const = A.INT (BigInt.fromInt const, loc), ty = BE.INTty,
                     loc = loc},
       BE.INTty)
  fun WordConst (const, loc) =
      (R.RCCONSTANT {const = A.WORD (const, loc), ty = BE.WORDty, loc = loc},
       BE.WORDty)
  ...
  fun Record (fields, loc) =
      let
        val fieldExps = SEnv.fromList (map (fn (l,(e,t)) => (l,e)) fields)
        val fieldTys = SEnv.fromList (map (fn (l,(e,t)) => (l,t)) fields)
  ...
  fun List (exps, loc) =
      let
        val (exps, tys) = ListPair.unzip exps
        val elemTy = List.hd tys
            handle Empty => raise Control.Bug "List"
        val listTyCon = BE.lookupTyCon BuiltinName.listTyName
        val listTy = T.CONSTRUCTty {tyCon = listTyCon, args = [elemTy]}
      in
        (R.RCLIST {expList = exps, listTy = listTy, loc = loc}, listTy)
      end
      ...
      let
        val columns =
            map (fn (label, ty) => compileColumn (label, ty, loc))
                (SEnv.listItemsi columnTyMap)
      in
        {table = List (map #column columns, loc),
      ...
      let
        val tables =
            map (fn (label, table) => compileTable (label, table, loc))
                (SEnv.listItemsi tableTyMap)
      in
        {schema = List (map #table tables, loc),
  ...
  fun compileServerDesc (fields, loc) =
      let
        val fields =
            map (fn (label, exp) => (label ^ " = ", (exp, BE.STRINGty))) fields
      in
        case fields of
          nil => StringConst ("", loc)
        | (label, exp)::t =>
          foldl
            (fn ((label, exp), result) =>
                StringConcat
                  (StringConcat (result,
                                 StringConst (" " ^ label ^ " = ", loc),
                                 loc),
                   exp, loc))
            (StringConcat (StringConst (label ^ " = ", loc), exp, loc))
            t
      end
