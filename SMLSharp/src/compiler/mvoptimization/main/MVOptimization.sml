...
structure MVOptimization : MVOPTIMIZATION = struct
  ...
  fun optimizeBasicBlock varEnv recordEnv commonExpEnv basicBlock =
      case basicBlock of
          MV.MVVALBLOCK {code, exnIDSet} =>
          let
              val (newCode,newVarEnv,newRecordEnv,newCommonExpEnv) =
                  optimizeDeclList varEnv recordEnv commonExpEnv code
          in
              (MV.MVVALBLOCK {code = newCode, exnIDSet = exnIDSet}, newVarEnv, newRecordEnv, newCommonExpEnv)
          end
        | MV.MVLINKFUNCTORBLOCK x => 
          (MV.MVLINKFUNCTORBLOCK x, varEnv, recordEnv, commonExpEnv)

  and optimizeBasicBlockList varEnv recordEnv commonExpEnv [] = 
      ([],varEnv,recordEnv,commonExpEnv)
    | optimizeBasicBlockList varEnv recordEnv commonExpEnv (basicBlock::basicBlockList) =
      let 
          val (basicBlock1,newVarEnv,newRecordEnv,newCommonExpEnv) = 
            optimizeBasicBlock varEnv recordEnv commonExpEnv basicBlock
          val (basicBlocks2,newVarEnv,newRecordEnv,newCommonExpEnv) =
            optimizeBasicBlockList newVarEnv newRecordEnv newCommonExpEnv basicBlockList
      in 
          (basicBlock1 :: basicBlocks2, newVarEnv, newRecordEnv, newCommonExpEnv)
      end

  fun optimizeTopBlock varEnv recordEnv commonExpEnv topBlock =
      case topBlock of
          MV.MVBASICBLOCK basicBlock =>
          let
              val (newBasicBlock, newVarEnv, newRecordEnv, newCommonExpEnv) =
                  optimizeBasicBlock varEnv recordEnv commonExpEnv basicBlock
          in
              (MV.MVBASICBLOCK newBasicBlock, newVarEnv, newRecordEnv, newCommonExpEnv)
          end
        | MV.MVFUNCTORBLOCK {name, bodyCode, formalAbstractTypeIDSet, formalVarIDSet, formalExnIDSet,
                             generativeExnIDSet, generativeVarIDSet} => 
          let
              val (newBodyCode,_,_,_) = 
                  optimizeBasicBlockList VarIdEnv.empty VarIdEnv.empty CommonExpEnv.empty bodyCode
          in
              (MV.MVFUNCTORBLOCK {name = name, 
  ...
  fun optimizeTopBlockList varEnv recordEnv commonExpEnv [] = 
      ([],varEnv,recordEnv,commonExpEnv)
    | optimizeTopBlockList varEnv recordEnv commonExpEnv (topBlock::topBlockList) =
      let 
          val (topBlock1,newVarEnv,newRecordEnv,newCommonExpEnv) = 
              optimizeTopBlock varEnv recordEnv commonExpEnv topBlock
          val (topBlocks2,newVarEnv,newRecordEnv,newCommonExpEnv) =
              optimizeTopBlockList newVarEnv newRecordEnv newCommonExpEnv topBlockList
      in 
