...
structure UselessCodeElimination : USELESSCODEELIMINATION = struct
  ...
  fun eliminateBasicBlockList vSet [] = ([], vSet)
    | eliminateBasicBlockList vSet (basicBlock :: basicBlockList) =
      let
          fun boundVarsInRecBinds recbinds =
              foldl
                  (fn ({boundVar, boundExp}, L) => boundVar::L)
          ...
          val (newBasicBlockList, newVarIdSet) = eliminateBasicBlockList vSet basicBlockList
      in
          case basicBlock of
              MVVALBLOCK {code, exnIDSet} =>
              let
                  val (newCode, varIdSet) =
                      foldr (fn (mvdecl, (newCode, newVarIdSet)) =>
                                case mvdecl of
                                    MVVAL {boundVars, boundExp, loc} =>
                                    let
                                        val boundVarIdSet = 
                                    ...
                                    in
                                        ...
                                        then (newCode, newVarIdSet)
                                            ...
                                            (
                                             MVVAL{boundVars = boundVars, boundExp = newBoundExp, loc = loc}::newCode,
                                             newVarIdSet2
                                            )
                                    end
                                  | MVVALREC {recbindList, loc} =>
                            ...
                            code
              in
                  ((MVVALBLOCK {code = newCode, exnIDSet = exnIDSet}) :: newBasicBlockList, 
                   varIdSet)
              end
            | MVLINKFUNCTORBLOCK x => 
              ((MVLINKFUNCTORBLOCK x) :: newBasicBlockList, newVarIdSet)
      end

  fun eliminateTopBlockList vSet [] = ([], vSet)
    | eliminateTopBlockList vSet (topBlock :: topBlockList) =
      let
          val (newTopBlockList, newVarIdSet) = eliminateTopBlockList vSet topBlockList
      in
          case topBlock of
              MVBASICBLOCK basicBlock =>
              (case eliminateBasicBlockList newVarIdSet [basicBlock] of
                   ([basicBlock], newVarIdSet) => 
                   (MVBASICBLOCK basicBlock :: newTopBlockList, newVarIdSet)
                 | _ => raise Control.Bug "must be singleton list")
            | MVFUNCTORBLOCK {name, bodyCode, formalAbstractTypeIDSet, formalVarIDSet, formalExnIDSet,
                              generativeExnIDSet, generativeVarIDSet} =>  
              let
                  val (decs, newVarIdSet) = eliminateBasicBlockList newVarIdSet bodyCode
              in
                  (MVFUNCTORBLOCK {name = name, 
                                   ...
                                   bodyCode = decs} :: newTopBlockList, 
                   newVarIdSet)
              end
      end

  fun optimize topBlocks = #1 (eliminateTopBlockList VarIdSet.empty topBlocks)
end
