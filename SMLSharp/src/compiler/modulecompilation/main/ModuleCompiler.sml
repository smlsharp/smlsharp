...
in
   ...
   fun constructDummyNamePath longid =
       let
           fun con nil path = raise Control.Bug (Absyn.longidToString(longid)^" is nil")
   ...
   fun compileOpenVarNameMap nameContext loc varNameMap = 
       SEnv.foldli (fn (varName, idstate, (flattenedVarNamePathEnv, newVarNameMap)) =>
                       case idstate of
   ...
   fun compileOpenTyNameMap nameContext loc tyNameMap =
       SEnv.foldli (fn (tyName, tyState, (tyNamePathEnv, newTyNameMap)) =>
                       case tyState of
                           NM.DATATY ((name, prefix), dataCon) =>
                           let
                               val leftConNameMap = constructDataCon (#strLevel nameContext) dataCon
                           in
                               (NPEnv.insert(tyNamePathEnv, 
   ...
   fun compileOpenStrNameMap nameContext loc strNameMap =
       SEnv.foldli (fn (strName, 
                        NM.NAMEAUX 
                            ...
                            ((flattenedTyNamePathEnv, flattenedVarNamePathEnv), newStrNameMap)) =>
                       let
                           val newParentPath = 
                               NM.joinPathWithWrapper (#strLevel nameContext) wrapperSysStructure
                           val newStrLevel = 
                               let
                                   val tempStrLevel =
                                       NM.appendStrLevel 
                                        ...
                                        NM.NAMEAUX 
                                            {
                                             ...
                                             basicNameMap = (newTyNameMap1, newVarNameMap1, newStrNameMap1)}))
                       end)
   ...
   and compileOpen nameContext loc (tyNameMap, varNameMap, strNameMap) =
       let
           ...
           val ((flattenedTyNamePathEnv2, flattenedVarNamePathEnv2), newStrNameMap) =
               compileOpenStrNameMap nameContext loc strNameMap
       in
           ((NM.mergeNamePathEnvs [flattenedTyNamePathEnv1, flattenedTyNamePathEnv2],
             NM.mergeNamePathEnvs [flattenedVarNamePathEnv1, flattenedVarNamePathEnv2]),
            (newTyNameMap, newVarNameMap, newStrNameMap))
       end

   (****************************************************************************)
   fun adjustPrefixVarNameMap strLevel varNameMap = 
       SEnv.foldli (fn (varName, idstate, newVarNameMap) =>
                       case idstate of
   ...
   fun adjustPrefixTyNameMap strLevel tyNameMap =
       SEnv.foldli (fn (tyName, tyState, newTyNameMap) =>
                       case tyState of
                           NM.DATATY ((name, prefix), dataCon) =>
                           let
   ...
   fun fixPrefixDataCons (prefix, datacon) =
       SEnv.map (fn NM.CONID (name, _) => NM.CONID (name, prefix)
                  | NM.VARID (namePath) => 
                    raise Control.Bug ((NM.namePathToString namePath)^" is constructor")
                  | NM.EXNID (namePath) => 
                    raise Control.Bug ((NM.namePathToString namePath)^" is constructor"))
                datacon

   fun constrainTyNameMap (richTyNameMap, strictTyNameMap) loc =
       SEnv.foldli (fn (tyName, strictTyInfo, newTyNameMap) =>
                         ...
                         | NM.NONDATATY _ =>
                           (case SEnv.find(richTyNameMap, tyName) of
                              ...
                              | SOME (NM.NONDATATY (name, prefix)) =>
                                SEnv.insert(newTyNameMap, tyName, NM.NONDATATY (name, prefix))))
                   SEnv.empty
                   strictTyNameMap

   fun constrainVarNameMap (richVarNameMap, strictVarNameMap) loc =
       SEnv.foldli (fn (varName, strictVarInfo, newVarNameMap) =>
   ...
   and constrainStrNameMapEntry 
           ((richEntry as NM.NAMEAUX {basicNameMap = (tm, vm, sm), name, wrapperSysStructure, parentPath, ...}),
            (strictEntry as (NM.NAMEAUX {basicNameMap=(tm', vm', sm'),...})))
           loc
           =
           let
   ...
   and constrainStrNameMap (richStrNameMap, strictStrNameMap) loc =
       SEnv.foldli (fn (strName, strictEntry, newStrNameMap) =>
                       case SEnv.find(richStrNameMap, strName) of
                         ...
                         | SOME richEntry =>
                           SEnv.insert(newStrNameMap, 
                                       strName, 
                                       constrainStrNameMapEntry  (richEntry, strictEntry) loc
                                      )
                           )
                   SEnv.empty
                   strictStrNameMap
   ...
   fun constrainBasicNameMap
       ...
       let
           ...
           val varNameMap = constrainVarNameMap (richVarNameMap, strictVarNameMap) loc
           val strNameMap = constrainStrNameMap (richStrNameMap, strictStrNameMap) loc
       in
           (tyNameMap, varNameMap, strNameMap)
       end

   fun constrainFunNameMap {rich : NameMap.funNameMap, strict : NameMap.funNameMap} loc =
       SEnv.foldli (fn (funName, {arg = strictArg, body = strictBody}, newFunNameMap) =>
                       case SEnv.find(rich, funName) of
                         ...
                         | SOME ({arg = richArg, body = richBody}) =>
                           let
                               ...
                               val newArg = constrainBasicNameMap {rich = strictArg, strict = richArg} loc
                               val newBody = constrainStrNameMapEntry (richBody, strictBody) loc
                           in
   ...
   fun constrainBasicInterfaceNameMap
           {rich : NameMap.basicInterfaceNameMap, strict : NameMap.basicInterfaceNameMap} loc =
       let
           val basicNameMap = constrainBasicNameMap {rich = #1 rich, strict = #1 strict} loc
           val funNameMap = constrainFunNameMap {rich = #2 rich, strict = #2 strict} loc
       in
       ...
       let
           ...
           fun patchSig (strexp, loc) sigExpSeq =
               (case sigExpSeq of
                    NONSIG => strexp
                  | TRANS (plsigexp, rem) =>
                    patchSig (PC.PLSTRTRANCONSTRAINT(strexp, plsigexp, loc), loc) rem
                  | OPAQUE (plsigexp, rem) =>
                    patchSig (PC.PLSTROPAQCONSTRAINT(strexp, plsigexp, loc), loc) rem)

           fun impl strExp sigExpSeq =
               case strExp of
                   PC.PLSTREXPBASIC (_, loc) => 
                   {
                    localPart = [PC.PLSTRUCTBIND ([(NAME_OF_ACTUAL_FUNCTOR_ARG, 
                 ...
                 | PC.PLSTRID(strNames, loc) =>
                   (case sigExpSeq of 
                      ...
                      | any => {localPart = [PC.PLSTRUCTBIND ([(Absyn.longidToString strNames, 
                                                                patchSig (strExp, loc) sigExpSeq)], 
                                                              loc)],
                 ...
                 | PC.PLFUNCTORAPP(_, _ , loc) =>
                   {localPart = [PC.PLSTRUCTBIND ([(NAME_OF_ACTUAL_FUNCTOR_ARG, 
                                                    patchSig (strExp, loc) sigExpSeq)], loc)],
                    namePart = [NAME_OF_ACTUAL_FUNCTOR_ARG]}
   ...
   fun abstractTyNameMapInNameMap (nameMap : NM.currentNameMap) =
       SEnv.map (fn NM.DATATY ((name, prefix), _) => NM.DATATY((name, prefix), SEnv.empty)
                  | NM.NONDATATY (name, prefix) => NM.NONDATATY (name, prefix))
                (#tyNameMap nameMap)

   (***************************************************)                               
   (*
    * The difference between Absyn.ty and PCF.ty is that string list of TYCONSTRUCT
    * is compiled into namePath.
    *)
   fun compileTy (nameContext:NM.nameContext) ty =
       case ty of
         ...
         | A.TYTUPLE (tys, loc) => A.TYTUPLE (map (compileTy nameContext) tys, loc)
         ...
         | A.TYPOLY (kindedTvarList, ty, loc) => 
           A.TYPOLY (compileKindedTvarList nameContext kindedTvarList, compileTy nameContext  ty, loc)
           
   and compileTvarKind nameContext tvarKind =
     case tvarKind of
       A.UNIV => A.UNIV
     | A.REC stringTyList => 
         A.REC ((map (fn (l,ty) => (l, compileTy nameContext ty))) stringTyList)

   and compileKindedTvar nameContext (tvar, tvarKind) = (tvar, compileTvarKind nameContext tvarKind)

   and compileKindedTvarList nameContext kindedTvarList = map (compileKindedTvar nameContext) kindedTvarList
...
*)
         ...
         | PC.PLHANDLE (plexp, plpatPlexpList, loc) => 
           let 
               ...
               val newPlpatPlexpList = 
                   foldr (fn ((plpat, plexp1), newPlrules) =>
                             let
         ...
         | PC.PLFNM (plpatListPlexpList , loc) => 
           let 
               val newPlrules = 
                   foldr (fn ((plpatList, plexp), plrules) =>
                             let
         ...
         | PC.PLCASEM (plexpList,  plpatListPlexpList, caseKind, loc) => 
           let 
               ...
               val plrules = 
                   foldr (fn ((plpatList, plexp1), plrules) =>
                             let
                                 val (newPlpatList, varNameMap) = 
                                     compilePatList nameContext plpatList
                                 val newNameContext =
         ...
         | PC.PLRECORD_SELECTOR (string , loc) => PCF.PLFRECORD_SELECTOR (string , loc)
         ...
         | PC.PLFFIAPPLY (callingConvention, plexp, ffiArgList, ty, loc) =>
           let
               ...
               val newTy = compileTy nameContext ty
           in
               PCF.PLFFFIAPPLY(callingConvention, newPlexp, newFfiArgList, newTy, loc)
           end
   ...
   and compileDatatypes nameContext datatypes =
           ...
           val newNameContext =
               let
                   val tyNameMap =
                       foldl (fn ((_, tyName, _), 
                                  tyNameMap) =>
                                 SEnv.insert(tyNameMap,
                                             tyName, 
                                             NM.DATATY((tyName, #strLevel nameContext), SEnv.empty)))
                             SEnv.empty
                             datatypes
               in
                   NM.extendNameContextWithCurrentNameMap
       ...
       in
           foldl (fn ((tvars, tyName:string, constructors), 
                     ...
                     let
                         ...
                         val (newConstructors, varNameMap) =
                             foldl (fn ((bool, constructorName, tyOpt), 
...
(*
                                                        if isAbsType then
                                                            NM.CONID (constructorName, Path.NilPath)
                                                        else
*)
                                   ...
                                   (nil, SEnv.empty)
                                   constructors
                         val newTyNameMap = SEnv.singleton(tyName, 
                                                           NM.DATATY((tyName, #strLevel nameContext),
                                                                     varNameMap))
                     in
   ...
   and compileDec (nameContext:NM.nameContext) pdecl = 
       case pdecl of
         PC.PDVAL (kindedTvarList, plpatPlexpList, loc ) => 
           let 
             val kindedTvarList = compileKindedTvarList nameContext kindedTvarList
             val (newPlpatPlexpList, varNameMap) = 
               foldl (fn ((plpat, plexp1), (plrules, varNameMap)) =>
                      let
       ...
       | PC.PDDECFUN (kindedTvarList, plpatPlpatListPlexpListList, loc)  => 
         let
             ...
             val newPlpatPlpatListPlexpListList =
                 foldl (fn ((funPat, plpatListPlexpList), newPlpatPlpatListPlexpListList)
                           =>
                           let
                               val (plFunPat, varNameMap1) = 
                                   compilePat nameContext funPat
                               val plruleMList = 
                                   foldr (fn ((plpatList, plexp), plruleMList) =>
                                             let
                                                 ...
                                                 val newNameContext1 =
                                                     NM.extendNameContextWithCurrentNameMap
                                                         {nameContext = newNameContext,
                                                          nameMap = 
                                                          NM.injectVarNameMapInNameMap varNameMap}
                                                 val newPlexp = compileExp newNameContext1 plexp
                                             in 
                                         ...
                                         plpatListPlexpList
                           in
                               newPlpatPlpatListPlexpListList @ [(plFunPat, plruleMList)]
                           end)
                       nil
                       plpatPlpatListPlexpListList
         in
             ([PCF.PDFDECFUN (kindedTvarList, newPlpatPlpatListPlexpListList, loc)],
         ...
         let
             ...
             val newPlruleMList = 
                 foldr (fn ((plpatList, plexp), plruleMList) =>
                       ...
                       plpatListPlexpList
         in
             ([PCF.PDFNONRECFUN (kindedTvarList, (newFunPat, newPlruleMList), loc)],
              NM.injectVarNameMapInNameMap varNameMap)
         end
       | PC.PDVALREC (kindedTvarList , plpatPlexpList , loc) => 
         ...
         | PC.PDTYPE (tvarsNameTyList, loc) =>
           let
               val (newTvarsNameTyList, tyNameMap) =
                   foldl (fn ((tyvars, tyName, ty), (tyFuns, tyNameMap)) =>
                             let
                                 val newTy = compileTy nameContext ty
                             in
                                 (tyFuns @ [(tyvars, 
         ...
         | PC.PDDATATYPE (datatypes,loc) =>  
           ...
           in 
               ([PCF.PDFDATATYPE (#strLevel nameContext, newDatatypes, loc)], nameMap)
           end
         | PC.PDABSTYPE (datatypes, pdeclList, loc) =>  
           let
               ...
               val newNameContext =
                   NM.extendNameContextWithCurrentNameMap {nameContext = nameContext,
                                                           nameMap = nameMap1}
               ...
               val newNameMap =
                   NM.mergeCurrentNameMap
                       {old = NM.injectTyNameMapInNameMap abstractTyNameMap,
                        new = nameMap2}
           in
               ([PCF.PDFABSTYPE(#strLevel nameContext, newDatatypes, newPdeclList, loc)], newNameMap)
           end
           ...
           let
               val (newRightLongName, varNameMap) =
                   case NM.lookupTy(nameContext, rightNames) of
                       ...
                       (E.enqueueError 
                            (loc, 
                             E.TyConNotFound({name = Absyn.longidToString(rightNames)}));
                            (constructDummyNamePath rightNames, SEnv.empty))
                     | (_, SOME (NM.DATATY (namePath, datacon))) => 
                       (namePath, adjustPrefixVarNameMap (#strLevel nameContext) datacon)
         ...
         | PC.PDEXD (exBinds, loc) =>
           let
               val (newExbinds, varNameMap) =
                   foldl (fn (exbind, (newExnBinds, varNameMap)) =>
                             case exbind of
                                 PC.PLEXBINDDEF (bool, name, tyOpt, loc) =>
                                 let
                                     val newTyOpt = compileTyOpt nameContext tyOpt
                                     val newNamePath = NM.constructNamePath nameContext name
                                 in
                                     (newExnBinds @ [PCF.PLFEXBINDDEF (bool, 
                                                                       newNamePath,
                                                                       newTyOpt, 
                                                                       loc)],
                                      NM.mergeVarNameMap
                               ...
                               | PC.PLEXBINDREP (bool1, leftName, bool2, rightNames, loc) =>
                                 let
                                     val newLeftNames = NM.constructNamePath nameContext leftName
                                     val newRightLongName = 
                                         case NM.lookupVar (nameContext, rightNames) of
                                             (_, NONE) => 
                                             (E.enqueueError 
                                                  (loc, 
                                                   E.ExnNotFound({name = Absyn.longidToString(rightNames)}));
         ...
         | PC.PDOPEN(strNameList, loc) => 
            ...
            *           val $1.S.x = 1;
            *           type $1.S.t = int;
            *           intro (varNamePathEnv = {$2.P.x -> $1.S.x} tyNamePathEnv =  {$2.P.t -> $1.S.t},
           ...
           let
               val (nameMap, newDecs) = 
                   foldl (fn (strName, (nameMap, newDecs)) =>
                             let
         ...
         | PC.PDINFIXDEC(n,idlist,loc) => ([PCF.PDFINFIXDEC(n, idlist, loc)], NM.emptyCurrentNameMap)
         | PC.PDINFIXRDEC(n,idlist,loc) => ([PCF.PDFINFIXRDEC(n,idlist,loc)], NM.emptyCurrentNameMap)
         | PC.PDNONFIXDEC(idlist,loc) => ([PCF.PDFNONFIXDEC(idlist,loc)], NM.emptyCurrentNameMap)
   ...
   and compileDeclList (nameContext:NameMap.nameContext) pdeclList = 
       let
           val (newPdeclList, _ , nameMap : NM.currentNameMap) =
               foldl (fn (pdecl, (newPdeclList, nameContext, nameMap:NM.currentNameMap)) =>
                         let
...
*)
         ...
         | PC.PLPATCONSTANT (constant, loc) => (PCF.PLFPATCONSTANT(constant,loc), SEnv.empty)
         ...
         | PC.PLPATRECORD (bool, stringPlpatList, loc) => 
           let
               val (fields, varNameMap) = 
                   foldr (fn ((l,plpat), (fields, varNameMap)) =>
                             let
                                 val (newPlpat, varNameMap') = 
                                     compilePat nameContext plpat
                             in 
                                 ((l, newPlpat)::fields,
                                  NM.mergeVarNameMap{new = varNameMap', old = varNameMap})
                             end)
                         (nil, SEnv.empty)
                         stringPlpatList
           in
         ...
         | PC.PLPATLAYERED (string , tyOpt , plpat , loc) => 
           let
               val varNameMap1 = SEnv.singleton (string, NM.VARID (string, Path.NilPath))
               val (newPlpat, varNameMap2) = compilePat nameContext plpat
               val newTyOpt = Option.map (compileTy nameContext) tyOpt
           in
   ...
   and compileStrDecs isTop nameContext strDecs = 
       let
           val (decs, _, nameMap) =
               foldl (fn (strDec, (decs, nameContext, incNameMap)) =>
                         let
         ...
         | PC.PLSTRUCTBIND (strBinds, loc) =>
           let
               val (units, incNameMap) = 
                   foldl (fn (strBind as (strName, strExp), (units, incNameMap)) => 
                             let
                                 val sysStructureName = VarNameGen.generate ()
                                 val sysStrpath = NM.appendStrLevel(#strLevel nameContext, 
                                                                    (sysStructureName, 
                                                                     NM.SYS))
                                 val newNameContext = 
                                     NM.updateStrLevel 
                             ...
                             in
                                 (units @ 
                                  [({strName = strName, 
                                     topSigConstraint = extractSigInfoFromTopStrExp isTop strExp,
...
*)
   ...
   and compileStrExp isTop nameContext strExp =
       case strExp of
         ...
         | PC.PLSTRID(strNames, loc) => 
           (* 
            *  structure identifier is transformed into a basic structure expression
            *  that contains only a open declaration.
            *)
           let
               val newStrExp =
                   PC.PLSTREXPBASIC ([PC.PLCOREDEC (PC.PDOPEN([strNames], loc), loc)], loc)
           in
         ...
         | PC.PLSTRTRANCONSTRAINT(plStrExp, plSigExp, loc) =>
            ...
            * 
            * Particularly, richNamePathEnv is used to restore the "clean" type environment
            * in the type inference to do signature matching.
            *)
           let
               val (decs, nameMap1) = 
           ...
           in 
               ([PCF.PDFTRANCONSTRAINT(decs, richFlattenedNameMap, sigSpec, strictFlattenedNameMap, loc)], 
                newNameMap)
           end
         | PC.PLSTROPAQCONSTRAINT(plStrExp, plSigExp, loc) =>
           let
               ...
               val newNameMap = NM.injectBasicNameMapInNameMap cutBasicNameMap
           in 
               ([PCF.PDFOPAQCONSTRAINT(decs, richFlattenedNameMap, sigSpec, strictFlattenedNameMap, loc)], 
                newNameMap)
           end
               ...
               val (actualArgPath, actualArgNameMap) = 
                   ...
                   in
                       case NM.lookupStr(newNameContext, actualArgName) of
                           ...
                           (E.enqueueError 
                                (loc, 
                                 E.StructureNotFound({name = Absyn.longidToString(actualArgName)}));
                   ...
                   let
                       val bodyBasicNameMap =
                           case NM.lookupFunctor(nameContext, funName) of
                             ...
                             | SOME ({body = NM.NAMEAUX {basicNameMap = bodyBasicNameMap,...}, ...}) => 
                               bodyBasicNameMap
                   in
                       adjustPrefixBasicNameMap (#strLevel nameContext) bodyBasicNameMap
                   end
               val newTerm = 
                   let
                       val term = [PCF.PDFFUNCTORAPP(#strLevel nameContext, 
                                                     funName, 
       ...
       case sigExp of
         ...
         | PC.PLSIGWHERE(sigExp, rlstns, loc) =>
           let
               ...
               val newRlstns = 
                   foldl (fn ((tvars, longid, ty), newRlstns) =>
                             let
                                 val (longPath, name) = 
                                     case NM.lookupTyInCurrentNameMap(incNameMap, longid) of
                                         (strPath, NONE) => 
                                         let
                                             val (name, strPath) = constructDummyNamePath longid
                                         in
                                             (E.enqueueError 
                                                  (loc, 
                                                   E.TyConNotFound({name = Absyn.longidToString(longid)}));
                                                  (strPath, name))
                                         end
                             ...
                             in
                                 (newRlstns @ [(tvars, 
                                                (name, Path.joinPath (#strLevel nameContext, longPath)),
       ...
       case spec of
           ...
           let
               val (newNamePathTys, varNameMap) = 
                   foldl (fn ((name, ty), (newNamePathTys, varNameMap)) =>
                             let
                                 val newNamePath = (name, (#strLevel nameContext))
                             in
                                 (newNamePathTys @ [(newNamePath, compileTy nameContext ty)],
                                  SEnv.insert
                                      (varNameMap,
                                       name, 
                         ...
                         nameTys
           in
               (PCF.PLFSPECVAL(newNamePathTys, loc), NM.injectVarNameMapInNameMap(varNameMap))
           end
         | PC.PLSPECTYPE(types, loc) => 
           let
               val (typeNameMap, newTypes) =
                   foldl (fn ((tyvars, name), (typeNameMap, newTypes)) =>
                             let
                                 val newName =  (name, (#strLevel nameContext))
                             in
                                 (SEnv.insert (typeNameMap, 
                                               name, 
                         ...
                         types
           in
               (PCF.PLFSPECTYPE(newTypes, loc), NM.injectTyNameMapInNameMap(typeNameMap))
           end
         | PC.PLSPECTYPEEQUATION ((tyvars, name, ty), loc) => 
           let
               ...
               val newTy = compileTy nameContext ty
               val tyNameMap =
                   SEnv.singleton(name, NM.NONDATATY(name, #strLevel nameContext))
           in
           ...
           let
               val newDatatypes = 
                   map
                       ...
                       datatypes
               val (nameMap, newDatatypes1) = compileDatatypes nameContext newDatatypes
               val newDatatypes2 = 
                   map
                       (fn (tvars, name, constructorNames) =>
                           let
                       ...
                       newDatatypes1
           in 
               (PCF.PLFSPECDATATYPE(#strLevel nameContext, newDatatypes2, loc), nameMap)
           end
                   ...
                   case NM.lookupTy(nameContext, rightNames) of
                       ...
                       (E.enqueueError 
                            (loc, 
                             E.TyConNotFound({name = Absyn.longidToString(rightNames)}));
                            (constructDummyNamePath rightNames, SEnv.empty))
                     | (_, SOME (NM.DATATY (namePath, varNameMap))) =>  (namePath, varNameMap)
                     | (_, SOME (NM.NONDATATY namePath)) => (namePath, SEnv.empty)
               val newVarNameMap = constructDataCon (#strLevel nameContext) varNameMap
               val nameMap =
                   NM.mergeCurrentNameMap
         ...
         | PC.PLSPECEXCEPTION(nameTyOpts, loc) => 
           let
               ...
               val (newNamePathTyOpts, nameMap) = 
                   foldl (fn ((name, tyOpt), (exns, nameMap)) =>
                             let
                                 val newNamePath = NM.constructNamePath newNameContext name
               ...
               val (specs, strNameMap) =
                   foldl (fn ((strName, sigExp), 
                             ...
                             let
                                 ...
                                 val (newSigExp, nameMap) = compileSigExp newNameContext sigExp
                                 val basicNameMap = NM.extractBasicNameMapFromNameMap nameMap
                             in
         ...
         | PC.PLSPECFUNCTOR (functorList, loc) =>
           let
               val (newFunctorList, functorNameMap) = 
                   foldl (fn ((funName, argSigExp, bodySigExp), (newFunctorList, functorNameMap)) =>
                             let
                                 (* functor specification  is the only one that involves unique structure name.
                                  * One problematic example imposes this requirement:
                                  * sig
                                 ...
                                 val sysStructureName = VarNameGen.generate ()
                                 val newNameContext = 
                                     NM.updateStrLevel (nameContext, 
                                                        Path.PSysStructure(sysStructureName, Path.NilPath))
                                 val (newArgSigExp, newArgNameMap) = 
                                     compileSigExp newNameContext argSigExp
                                 val basicArgSigNameMap = NM.extractBasicNameMapFromNameMap newArgNameMap
                                 val newNameContext = 
                                     NM.extendNameContextWithCurrentNameMap
                                         {nameContext = nameContext, 
                                          nameMap = NM.injectBasicNameMapInNameMap basicArgSigNameMap}
                                 val sysStructureName = VarNameGen.generate ()
                                 val newNameContext = 
                             ...
                             in
                                 (newFunctorList @ [(funName, 
                                                     (newArgSigExp, 
                                                      NM.basicNameMapToFlattenedNamePathEnv basicArgSigNameMap),
                                                     (newBodySigExp,
                                                      NM.basicNameMapToFlattenedNamePathEnv basicBodyNameMap)
                                                    )
                                                   ],
                                  SEnv.insert(functorNameMap,
                                              funName,
                                               ...
                                               body = NM.NAMEAUX
                                                          {
                                                           ...
                                                           basicNameMap = NM.extractBasicNameMapFromNameMap bodyNameMap
                                                          }
                                              }
                                             )
                                 )
                         ...
                         functorList
           in
               (PCF.PLFSPECFUNCTOR (newFunctorList, loc), NM.injectFunNameMapInNameMap functorNameMap)
           end
*)
         ...
         | PC.PLSPECSHARE(spec, longTyConNames, loc) => 
           let
               ...
               val longTyConPaths = 
                   map (fn longTyConName =>
                           case NM.lookupTyInCurrentNameMap (nameMap, longTyConName) of
                               (_, NONE) => 
                               (E.enqueueError 
                                    (loc, 
                                     E.TyConNotFound({name = Absyn.longidToString(longTyConName)}));
                                    (constructDummyNamePath (longTyConName)))
                             | (_, SOME (NM.DATATY((name, strPath), _))) => (name, strPath)
                             | (_, SOME (NM.NONDATATY(name, strPath))) => (name, strPath))
                       longTyConNames
           in
         ...
         | PC.PLSPECSHARESTR(spec, longStrNames, loc) => 
           let
                   ...
                   map (fn longStrName => 
                           let
                               val (longStrPath, strEntry) = 
                                   case NM.lookupStrInCurrentNameMap (nameMap, longStrName) of
                                       (_, NONE) => 
                                       (E.enqueueError 
                                            (loc, 
                                             E.StructureNotFound
                                                 ({name = Absyn.longidToString(longStrName)}));
                                            let
                                                val strpath = constructDummyPath longStrName
                                                val name = Path.getLastElementOfPath strpath
                                            in
                                                (strpath, NM.NAMEAUX {name = name, 
                                                                      wrapperSysStructure = NONE,
                                                                      parentPath = Path.NilPath,
                                                                      basicNameMap = 
                                                                      (SEnv.empty, SEnv.empty,SEnv.empty)})
                                            end)
                                     | (strPath, SOME (strEntry as (NM.NAMEAUX {name,  ...}))) => 
                                       (Path.appendUsrPath(strPath, name), strEntry)
                           in
                               (strEntry, Path.joinPath(#strLevel nameContext, longStrPath))
                           end)
                       longStrNames
                       
               fun share nil sharings = sharings
                 | share ((strNameMapEntry, longStrPath) :: mixedEntries) sharings = 
                   let
                       val sharings1 = 
                       ...
                       (fn (strName, 
                           ...
                           case SEnv.find(strMap2, strName) of
                             ...
                             | SOME (NM.NAMEAUX {
                               ...
                               let
                                   val pairPath = 
                                       (NM.appendStrLevel (longStrPath1, (strName, NM.USR)),
                                        NM.appendStrLevel (longStrPath2, (strName, NM.USR)))
                                   val sharings3 = 
               ...
               val newTerm = 
                   ...
                   in
                       foldl (fn ((leftLongTyConName, rightLongTyConName), term) => 
                                 PCF.PLFSPECSHARE(term,
   ...
   and compileSigDecs nameContext sigDecs = 
       let
           val (newSigDecs, _, sigNameMap) =
               foldl (fn (sigDec, (newSigDecs, newNameContext, sigNameMap)) =>
                         let
         ...
         | PC.PLTOPDECFUN(funBinds, loc) => 
           let
               val (newFunBinds, funNameMap) = 
                   foldl (fn ((funName, argStrName, argSigExp, bodyStrExp, loc), 
                              (funBinds, incNameMap)) => 
                             let
                                 val sysStructureName = VarNameGen.generate ()
                                 val argStrPath = 
                                 ...
                                 val bodySigExpOpt = extractSigInfoFromTopStrExp true bodyStrExp
                             in
                                 ((funBinds @ 
                                   [(funName, 
                                     (newArgSpec, 
                                      argStrName,
                                      NM.basicNameMapToFlattenedNamePathEnv argBasicNameMap,
                                      argSigExp), 
               ...
               foldl
                   (fn (topDec, (topDecs, incNameMap, nameContext)) =>
                       ...
                       in
                           (
                            topDecs @ [newTopDec],
                            NM.mergeCurrentNameMap {new = incNameMap', old = incNameMap},
                            NM.extendNameContextWithCurrentNameMap {nameContext = nameContext,
                                                                    nameMap = incNameMap'}
                            )
                       end)
