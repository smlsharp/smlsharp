 ...
 * @version $Id: SubFunctions.sml,v 1.5 2008/08/06 17:23:40 ohori Exp $
 *)
...
fun takeVariables2 (clusterCode : clusterCode) =
    let val vars = #frameInfo clusterCode
        val pointers = #pointers vars
        val atoms = #atoms vars
        val doubles = #doubles vars
    ...
    in
        ((pt_end,at_end,db_end), (pointers, atoms, doubles),
         (Vector.fromList (pointers @  atoms @ doubles @ (flatten (#records vars)))), (flatten (#records vars)))
    end
