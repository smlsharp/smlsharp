 ...
 * @version $Id: Liveness.sml,v 1.12 2007/12/23 17:11:43 matsu Exp $
 *)
...
fun arrayToList (varVector,arr) = Array.foldr (fn (a,R) => (map (curryNth varVector) (BitArray.getBits a)) :: R) [] arr
...
fun livenessAnalysis (bcfg,bbs,def,use',vars,var_map,prog) =
    let 
        ...
        val inLives = Array.tabulate (bbsLn,fn _ => BitArray.array (lnv, false))
        val outLives = Array.tabulate (bbsLn, fn _ => BitArray.array (lnv,false))
        ...
        fun addExceptionLiveness () = 
            let fun aux  (i,head) = 
                    ...
                    in List.app (fn n => BitArray.union (Array.sub(inLives,i)) (Array.sub (outLives,n))) excEntryBlocks
                    end
            in Vector.appi aux (headBBs(*,0,NONE*))
            end

        fun setBitsList (l,bitArray) = 
            app (fn n => BitArray.setBit (bitArray,n)) l
        fun clearBitsList (l,bitArray) =
            app (fn n => BitArray.clrBit (bitArray,n)) l
 
        fun makeUseDefInBlock bb =
            let
              val defBits = BitArray.array (lnv, false)
              val useBits = BitArray.array (lnv, false)
            in
              app (fn n =>
                      let
                  ...
                  bb;
              BitArray.complement defBits;
              (useBits, defBits)
            end

        val blockUDs = Vector.map makeUseDefInBlock listBBs
        
        fun clearBitArray bar = BitArray.appi (fn (i,_) => BitArray.clrBit (bar,i)) bar
            
        fun transferBlock ((blockUse,notBlockDef), analysis) = 
            let val tmp = BitArray.andb (analysis, notBlockDef, lnv)
            in (BitArray.union tmp blockUse; tmp)
            end
            ...
            (while (not (!Wx = nil))
             do
                 let val (lp,l) = hd (!Wx)
                     ...
                     val lv_lSet = IntBinarySet.addList (IntBinarySet.empty,BitArray.getBits lv_l)
                     val flp_Set = IntBinarySet.addList (IntBinarySet.empty,BitArray.getBits flp)
                 in 
                     if IntBinarySet.isSubset (flp_Set,lv_lSet)
                     then ()
                     else (flag := true;
                           BitArray.union lv_l flp;
