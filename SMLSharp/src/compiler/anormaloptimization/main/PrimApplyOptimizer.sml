 ...
 * @version $Id: PrimApplyOptimizer.sml,v 1.10 2007/10/29 10:12:55 kiyoshiy Exp $
 *)
structure PrimApplyOptimizer = struct
  ...
  fun floatToString r = Float.toString r
  ...
  fun stringToFloat s =
      case float.fromString s of
        SOME r => r
      | _ => raise Control.Bug "a float value is expected"
  ...
  fun genericOp_lili_li constOp atomEnv primApplyInfo =
      genericOp_2
          (fn (CT.LARGEINT v1,CT.LARGEINT v2) => CT.LARGEINT (constOp(v1,v2)))
          atomEnv
          primApplyInfo
  ...
  fun genericOp_ff_f constOp atomEnv primApplyInfo =
      genericOp_2
          (fn (CT.FLOAT v1,CT.FLOAT v2) => 
              CT.FLOAT (floatToString(constOp(stringToFloat v1,stringToFloat v2))))
          atomEnv
          primApplyInfo
  ...
  fun genericOp_lili_l constOp atomEnv primApplyInfo =
      genericOp_2
          (fn (CT.LARGEINT v1,CT.LARGEINT v2) => 
              if constOp(v1,v2) then trueValue else falseValue)
          atomEnv
          primApplyInfo
  ...
  fun genericOp_ff_l constOp atomEnv primApplyInfo =
      genericOp_2
          (fn (CT.FLOAT v1,CT.FLOAT v2) => 
              if constOp(stringToFloat v1,stringToFloat v2) then trueValue else falseValue)
          atomEnv
          primApplyInfo
  ...
  fun genericOp2_lili_li constOp atomEnv primApplyInfo =
      genericOp2_2
          (fn (CT.LARGEINT v1,CT.LARGEINT v2) => CT.LARGEINT (constOp(v1,v2)))
          atomEnv
          primApplyInfo
  ...
  fun genericOp2_ff_f constOp atomEnv primApplyInfo =
      genericOp2_2
          (fn (CT.FLOAT v1,CT.FLOAT v2) => 
              CT.FLOAT (floatToString(constOp(stringToFloat v1,stringToFloat v2))))
          atomEnv
  ...
  fun genericOp2_lili_l constOp atomEnv primApplyInfo =
      genericOp2_2
          (fn (CT.LARGEINT v1,CT.LARGEINT v2) => 
              if constOp(v1,v2) then trueValue else falseValue)
          atomEnv
  ...
  fun genericOp2_ff_l constOp atomEnv primApplyInfo =
      genericOp2_2
          (fn (CT.FLOAT v1,CT.FLOAT v2) => 
              if constOp(stringToFloat v1,stringToFloat v2) then trueValue else falseValue)
          atomEnv
  ...
  fun genericOp_f_f constOp atomEnv primApplyInfo =
      genericOp_1
          (fn (CT.FLOAT v) => 
              CT.FLOAT (floatToString (constOp (stringToFloat v))))
          atomEnv
          primApplyInfo
  ...
  fun genericOp_f_i constOp atomEnv primApplyInfo =
      genericOp_1
          (fn (CT.FLOAT v) => 
              CT.INT (constOp (stringToFloat v)))
          atomEnv
          primApplyInfo
  ...
  fun genericOp_i_f constOp atomEnv primApplyInfo =
      genericOp_1
          (fn (CT.INT v) => 
              CT.FLOAT (floatToString (constOp  v)))
          atomEnv
          primApplyInfo
  ...
  local
    fun divRealNumOp atomEnv (primOp,[argExp1,argExp2],loc) =
        let
          val arg1 = anexpToPrimArg atomEnv argExp1
          val arg2 = anexpToPrimArg atomEnv argExp2
        in
          (* Because exp has been type checked in earlier phase,
           * primOp, argExp1 and argExp2 are matched correctly.
           *)
          case (arg1,arg2) of
          ...
          | (ARGCONST (CT.FLOAT f1),ARGCONST (CT.FLOAT f2)) =>
            (case floatToString((stringToFloat r1)/(stringToFloat r2)) of
               "nan" => makePrimApply(primOp,[argExp1,argExp2],loc)
             | "inf" => makePrimApply(primOp,[argExp1,argExp2],loc)
             | "~inf" => makePrimApply(primOp,[argExp1,argExp2],loc)
             | s => ANCONSTANT {value=CT.FLOAT s,loc=loc})
          | (ARGCONST c1,ARGVAR v2) => makePrimApply1(primOp,c1,v2,loc)
          | (ARGVAR v1,ARGCONST c2) => makePrimApply2(primOp,v1,c2,loc)
          | (ARGVAR v1,ARGVAR v2) => makePrimApply(primOp,[v1,v2],loc)
        end
  in
  val divRealOp = divRealNumOp
  val divFloatOp = divRealNumOp
  end
  ...
  val optimizers = 
      ...
      [
       ...
       ("Float_fromInt",genericOp_i_f (Float.fromLargeInt)),
       ("Float_floor",genericOp_f_i (fn r => Int32.fromInt (Float.floor r))),
       ("Float_ceil",genericOp_f_i (fn r => Int32.fromInt (Float.ceil r))),
       ("Float_trunc",genericOp_f_i (fn r => Int32.fromInt (Float.trunc r))),
       ("Float_round",genericOp_f_i (fn r => Int32.fromInt (Float.round r))),
