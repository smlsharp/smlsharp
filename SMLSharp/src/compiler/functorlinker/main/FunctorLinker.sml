...
in
  ...
  val pu_functorEnv = EnvPickler.SEnv (Pickle.list MultipleValueCalcPickler.mvdecl)
  ...
  fun extendLinkContextWithTemplateRenamingVarBasis (lc:linkContext, trvb) =
      {
       ...
       VIC.mergeTemplateVarRenamingBasis {old = #templateRenamingVarBasis lc,
                                                    new = trvb}
       }
      ...
      case ty of
        ...
        | AT.FUNMty {argTyList, bodyTy, funStatus, annotation} =>
          AT.FUNMty {argTyList = map (linkTemplateTy linkContext) argTyList, 
                       ...
                       funStatus = funStatus}
        | AT.MVALty tyList => AT.MVALty (map (linkTemplateTy linkContext) tyList)
        | AT.RECORDty {fieldTypes, annotation} =>
          AT.RECORDty {fieldTypes = SEnv.map (linkTemplateTy linkContext) fieldTypes, 
                       annotation = annotation}
        | AT.POLYty {boundtvars, body} =>
          AT.POLYty {boundtvars = IEnv.map (linkTemplateBtvKind linkContext) boundtvars, 
                     body = linkTemplateTy linkContext body}
        | AT.RAWty {tyCon, args} =>
          AT.RAWty {tyCon = tyCon, args = map (linkTemplateTy linkContext) args}
  ...
  and linkTemplateBtvKind linkContext {id, recordKind, eqKind, instancesRef} =
      let
          ...
          val _ = instancesRef := map (linkTemplateTy linkContext) (!instancesRef)
      in
          {
           id = id,
  ...
  fun linkTemplateIndex (linkContext:linkContext) index =
      case ExternalVarID.Map.find(#externalVarIDResolutionTable linkContext, index) of
          SOME newIndex => newIndex
        | NONE => 
          case ExternalVarID.Map.find(#refreshedExternalVarIDTable linkContext, index) of
              SOME newIndex => newIndex
            | NONE => index
  ...
  fun linkTemplateExceptionTag (linkContext:linkContext) tag loc =
      case ExnTagID.Map.find(#exnTagResolutionTable linkContext, tag) of
          SOME newTag => newTag
        | NONE =>
          case ExnTagID.Map.find(#refreshedExceptionTagTable linkContext, tag) of
              SOME newTag => newTag
            | NONE => tag
  ...
  and linkTemplateDecs linkContext templateCode =
      let
          val (incTemplateVarRenamingBasis, _, mvdecs) =
              foldl (fn (mvdec, (incTemplateRenamingVarBasis, newLinkContext, mvdecs)) =>
                        let
      ...
      case mvdec of
          ...
          let
              val (newBoundVars, newTemplateVarRenamingBasis) = 
                  foldl (fn ({displayName, ty, varId = T.INTERNAL id}, 
                             (newBVs, newTVB)) =>
                            let
                                val newId = Counters.newLocalId()
                            ...
                            in
                                (newBVs @ [{
                                 ...
                                 VIC.mergeTemplateVarRenamingBasis
                                     {new = LocalVarID.Map.singleton(id, (displayName, newId)),
                                      old = newTVB})
                            end
                        ...
                        boundVars
              val newLinkContext = 
                  extendLinkContextWithTemplateRenamingVarBasis (linkContext, newTemplateVarRenamingBasis)
        ...
        | MVVALREC {recbindList, loc} =>
          let
              val incTemplateVarRenamingBasis = 
                  foldr (
                         fn ({boundVar = {displayName, ty, varId = T.INTERNAL oldId}, boundExp}, 
                             templateVarRenamingBasis) => 
                            let
                                val newId = Counters.newLocalId()
                            in
                                VIC.mergeTemplateVarRenamingBasis
                                    {new = LocalVarID.Map.singleton(oldId, (displayName, newId)),
                                     old = templateVarRenamingBasis}
                            end
                          | ({boundVar = {displayName, ty, varId = T.EXTERNAL index}, boundExp}, 
                             templateVarRenamingBasis) =>
                            templateVarRenamingBasis
                            )
                        ...
                        recbindList
              val newLinkContext = 
                  extendLinkContextWithTemplateRenamingVarBasis (linkContext, incTemplateVarRenamingBasis)
              val newRecbindList =
                  foldr ( 
                         fn ({boundVar = {displayName, ty = oldTy, varId = T.INTERNAL id}, boundExp},
                             newRecbindList) =>
                            let
                                val newId = 
                                ...
                                val newMvexp = linkTemplateMvexp newLinkContext boundExp
                            in
                                {boundVar = newBoundVar, boundExp = newMvexp} :: newRecbindList
                            end
                          | ({boundVar = {displayName, ty = oldTy, varId = T.EXTERNAL index}, boundExp},
                             newRecbindList) =>
                            let
                                val newIndex = linkTemplateIndex linkContext index
  ...
  and linkTemplateMvexp linkContext mvexpression =
      case mvexpression of
          MVFOREIGNAPPLY {funExp=foreignFunInfo, 
                          ...
                          attributes, 
                          loc} => 
          MVFOREIGNAPPLY {funExp = linkTemplateMvexp linkContext foreignFunInfo,
        ...
        | MVEXCEPTIONTAG {tagValue, loc} =>
          MVEXCEPTIONTAG {tagValue = linkTemplateExceptionTag linkContext tagValue loc, loc = loc}
        | MVVAR ({varInfo ={displayName, ty, varId = varId as (T.EXTERNAL index)}, loc}) => 
          MVVAR ({varInfo ={displayName = displayName,
        ...
        | MVCOPYARRAY {srcExp, srcIndexExp, dstExp, dstIndexExp, lengthExp, elementTy, loc} =>
          MVCOPYARRAY {srcExp = linkTemplateMvexp linkContext srcExp,
        ...
        | MVPRIMAPPLY {primInfo = {name, ty}, argExpList, instTyList, loc} =>
          MVPRIMAPPLY{primInfo = {name = name, ty = linkTemplateTy linkContext ty},
        ...
        | MVHANDLE {exp,
          ...
          let
              val id = case varId of 
                           T.INTERNAL id => id
                         | T.EXTERNAL _ => raise Control.BugWithLoc ("expect local variable in handler",loc)
              val newId =  Counters.newLocalId()
              val newVar  = 
                  { 
                   displayName = displayName,
              ...
              val newTemplateVarRenamingBasis = LocalVarID.Map.singleton(id, (displayName, newId))
              val newLinkContext = 
                  extendLinkContextWithTemplateRenamingVarBasis (linkContext, newTemplateVarRenamingBasis)
          in
                  ...
                  map (fn {displayName, ty, varId = T.INTERNAL id} =>
                          (id,
                           { 
                            displayName = displayName,
                            ty = linkTemplateTy linkContext ty,
                            varId = T.INTERNAL (Counters.newLocalId())
                            })
                        | {displayName, ty, varId = T.EXTERNAL _} =>
                          raise Control.BugWithLoc ("expect local variable in function argument",loc)
                          )
                      argVarList
              val newTemplateVarRenamingBasis = 
                  foldl 
                      (fn ((id, {displayName, ty, varId = T.INTERNAL newId}), templateVarRenamingBasis) =>
                          VIC.mergeTemplateVarRenamingBasis
                              {old = templateVarRenamingBasis,
                               new = LocalVarID.Map.singleton(id, (displayName, newId))}
          ...
          let
              val templateCode = 
                  case SEnv.find(functorEnv, name) of
                      NONE => raise Control.BugWithLoc ("unbound functor "^name, loc)
                    | SOME code => code
              val linkContext = 
                  {
  ...
  fun link functorEnv (stamp:Counters.stamp) topBlocks =
      let
          val _ = Counters.init stamp
          val (_, incFunctorEnv, newTopBlocks) =
              foldl (fn (topBlock, (newFunctorEnv, incFunctorEnv, newTopBlocks)) =>
                        let
                    ...
                    topBlocks
      in (incFunctorEnv, Counters.getCounterStamp(), newTopBlocks) end
end
end
