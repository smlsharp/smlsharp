...
struct
  ...
  fun makeRecordTy fields =
      T.RECORDty {fieldTypes = SEnv.fromList fields,
  ...
  fun freeVarsPrim bv baprim =
      ...
      | A.BARECORD {fieldList, recordTy, annotation, isMutable,
...
*)
  ...
  fun computeFrameBitmap env (btvEnv, argVarList, freeTyvars, loc) =
      if BoundTypeVarID.Set.isEmpty freeTyvars then
         ...
         * all the ftv must be already in the current frameBitmap. *)
        let
