...
structure DatatypeCompilation : sig
  ...
  datatype representation = WORDty | BOXEDty
  val representationOf : Types.tyCon -> representation
...
struct
        ...
        val (binds, fields, tys) =
            ListPair.foldrEq
              (fn (label, term, (binds, fields, tys)) =>
                  ...
                  in
                    (binds2 @ binds,
                     SEnv.insert (fields, label, exp),
                     SEnv.insert (tys, label, ty))
                  end)
              (nil, SEnv.empty, SEnv.empty)
              (labels, terms)
      ...
      in
        LET (BIND binds,
             EXP (fn loc => TL.TLRECORD
                              {isMutable = false,
                               fields = SEnv.map (fn f => f loc) fields,
                               recordTy = recordTy,
                               loc = loc},
                  recordTy))
      end
  ...
  fun explodeRecordExp term =
      let
        ...
        val fields =
            case TypesUtils.derefTy (termTy term) of
              T.RECORDty tys =>
              map (fn label => SELECT (label, term)) (SEnv.listKeys tys)
            | _ => [term]
      in
        (binds, fields)
      end
  ...
  fun explodeRecordTy ty =
      case TypesUtils.derefTy ty of
        T.RECORDty fields => ListPair.unzip (SEnv.listItemsi fields)
      | _ => raise Control.Bug "explodeRecordTy"

  fun tupleTy tys =
      T.RECORDty (SEnv.fromList (mapi (fn (i,ty) => (Int.toString i, ty)) tys))
  ...
  fun datatypeLayout ({id, conSet, ...}:T.tyCon) =
  ...
  fun representationOf tyCon =
      case datatypeLayout tyCon of
        LAYOUT_TAGGED (TAGGED_RECORD _) => BOXEDty
      | LAYOUT_TAGGED (TAGGED_TAGONLY _) => WORDty
      | LAYOUT_TAGGED (TAGGED_OR_NULL _) => BOXEDty
      | LAYOUT_BOOL _ => WORDty
      | LAYOUT_UNIT => WORDty
      | LAYOUT_ARGONLY => BOXEDty
      | LAYOUT_ARG_OR_NULL => BOXEDty
      | LAYOUT_REF => BOXEDty
  ...
  fun compileExp (env:env) rcexp =
          ...
          fun explodeArgTy ty =
              ...
              | T.FUNMty ([argTy], retTy) =>
                (case TypesUtils.derefTy argTy of
                   T.RECORDty fields => T.FUNMty (SEnv.listItems fields, retTy)
      ...
      | RC.RCDATACONSTRUCT {con, instTyList, argExpOpt, loc} =>
        ...
        in
          unpack (composeCon (con, instTyList, argExpOpt), loc)
        end
      | RC.RCEXNCONSTRUCT {exn, instTyList=nil, argExpOpt, loc} =>
        let
          val argExpOpt = Option.map (compileExp env) argExpOpt
        in
          unpack (composeExn env (exn, argExpOpt), loc)
        end
      ...
      | RC.RCRECORD {fields, recordTy, loc} =>
        TL.TLRECORD
          {isMutable = false,
           fields = SEnv.map (compileExp env) fields,
           recordTy = recordTy,
           loc = loc}
      ...
      | RC.RCLIST {expList, listTy, loc} =>
        let
          val elemTy = case TypesUtils.derefTy listTy of
                         T.CONSTRUCTty {tyCon, args=[ty]} => ty
                       | _ => raise Control.Bug "compileExp: RCLIST"
          val consConInfo = BE.lookupCon BuiltinName.consConName
          val nilConInfo = BE.lookupCon BuiltinName.nilConName
          val newExp =
              foldr (fn (exp, z) =>
                        RC.RCDATACONSTRUCT {con = consConInfo,
                                            instTyList = [elemTy],
                                            argExpOpt = SOME exp,
                                            loc = loc})
                    (RC.RCDATACONSTRUCT {con = nilConInfo,
                                         instTyList = [elemTy],
                                         argExpOpt = NONE,
                                         loc = loc})
                    expList
        in
          compileExp env newExp
        end
  ...
  and compileDecl env rcdecl =
      ...
      | RC.RCEXD (exnBinds, loc) =>
        let
          fun allocTagExp path =
              let
                val name = String.concatWith "." path
                val nameExp = TL.TLCONSTANT {value = CT.STRING name, loc = loc}
              in
                CAST (BE.EXNTAGty, RefAlloc (pack (nameExp, BE.STRINGty)))
              end
              ...
              in
                (env,
                 TL.TLVAL {boundVar = tagVar,
                           boundExp = unpack (allocTagExp (#path exnInfo), loc),
                           loc = loc}
                 :: decls)
              end
        in
      ...
      | RC.RCEXPORTEXN (exnInfo, loc) =>
        (
          ...
          | SOME var => (env, [TL.TLEXPORTVAR (var, loc)])
        )
