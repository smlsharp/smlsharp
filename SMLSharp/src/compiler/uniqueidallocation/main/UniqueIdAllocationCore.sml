        ...
        let
            val (varIDEnv,tfppatlist) = 
                ...
                fn (tppat,(varIDEnv,tfppatlist)) =>
                   let
                       val (varIDEnv',tfppat) = tppatToTfppat NPEnv.empty context tppat
                       val newvarIDEnv = 
...
and tppatToTfppat orEnv context tppat =
    case tppat of
      ...
      | TPPATVAR (varPathInfo as {namePath,ty,...},loc) =>
        let
            val name = 
                 ...
                 * For the error reporting of Match Compiler, since all variables in pattern only have
                 * short names, we just throw away the path part in namePath.
                 *)
                #1 namePath
            val id = 
                case NPEnv.find(orEnv, namePath) of 
                    SOME (VIC.Internal (id, _)) => id
                  | SOME (VIC.External _ ) => raise Control.Bug "illegal topItem here"
      ...
      | TPPATDATACONSTRUCT {conPat=conPathInfo as {namePath,funtyCon,ty,tag,tyCon},
                            instTyList=tys,
                            argPatOpt=tppatop,
                            patTy=ty1,
                            loc=loc} =>
        ...
        in
            (
             varIDEnv,
             TFPPATDATACONSTRUCT{conPat=newconInfo,instTyList=tys,argPatOpt=tfppatop,patTy=ty1,loc=loc}
            ) 
        end
      | TPPATEXNCONSTRUCT {exnPat=conPathInfo as {namePath,funtyCon,ty,tag,tyCon},
                           instTyList=tys,
                           argPatOpt=tppatop,
                           patTy=ty1,
                           loc=loc} =>
        ...
        in
            (
             varIDEnv,
             TFPPATEXNCONSTRUCT{exnPat=newconInfo,instTyList=tys,argPatOpt=tfppatop,patTy=ty1,loc=loc}
            ) 
        end
      ...
      | TPPATORPAT (tppat1, tppat2, loc) => 
        let
            val (varIDEnv1,tfppat1) = tppatToTfppat orEnv context tppat1
            val (varIDEnv2,tfppat2) = tppatToTfppat (VIC.mergeVarIDEnv {newVarIDEnv = varIDEnv1,
                                                                        oldVarIDEnv = orEnv})  
                                                    context tppat2
        in
            (varIDEnv1,
...
and tpexpToTfpexp (context:UIAC.context) (tpexp : tpexp) =
    case tpexp of
        ...
        let 
            ...
            val tfpexp1 = 
                case VIC.lookupVar (#topVarExternalVarIDBasis context, 
                  ...
                  | NONE =>
                    raise Control.BugWithLoc ("undefined variable:"^ (NM.namePathToString namePath),
                                              loc)
        in
            tfpexp1
        end
      ...
      | TPDATACONSTRUCT{con=conPathInfo as {namePath,funtyCon,ty,tag,tyCon},
        ...
        in
            TFPDATACONSTRUCT{con=newconInfo, instTyList=tys, argExpOpt=tfpexpop, loc=loc}
        end
      | TPEXNCONSTRUCT{exn=exnPathInfo as {namePath,funtyCon,ty,tag,tyCon},
        ...
        in
            TFPEXNCONSTRUCT{exn=newexnInfo, instTyList=tys, argExpOpt=tfpexpop, loc=loc}
        end
      ...
      | TPSELECT {label=tag, exp=tpexp, expTy=ty, resultTy, loc=loc} =>
        let
            val tfpexp = tpexpToTfpexp context tpexp 
        in 
            TFPSELECT{label=tag, exp=tfpexp, expTy=ty, resultTy= resultTy, loc=loc}
        end
      | TPMODIFY {label, 
                  recordExp=tpexp1, 
                  recordTy=ty1, 
...
and tpdecToTfpdecs (context:UIAC.context) tpdec =
    case tpdec of
      ...
      | TPVALREC (decs,loc) =>
        let 
            val (recVarIDEnv, exportVarIDEnv, exportReplicationDecs) = 
                foldr (fn ({var={namePath,ty},...}, (recVarIDEnv, exportVarIDEnv, exportDecs)) => 
                          let
                              val name = NM.usrNamePathToString namePath
            ...
            fun compiledecs context nil = nil
              | compiledecs context (dec::rem) = 
                let
                    ...
                    val name = NM.usrNamePathToString namePath
                    val curItem = 
                        case VIC.lookupVarInVarIDBasis(#varIDBasis context, namePath) of
                    ...
                    (fn (namePath, idstate, newVarIDEnv) =>
                        case idstate of
                            ...
                            (case VIC.lookupVar (#topVarExternalVarIDBasis context, 
                                                 #varIDBasis context, 
                                                 actualNamePath) of
                                 NONE => raise Control.BugWithLoc ("unbound variable:"^
      ...
      | TPEXNDEC (exnBinds,loc) => 
        let
            val exnInfos = foldr (fn (exnBind, newExnInfos) =>
                                     case exnBind of
      ...
      | TPFUNDECL (fundecl,_) =>
        (map  (fn {funVar = {namePath,...},...} => (print (NM.namePathToString namePath); print "\n"))
              fundecl;
