  ...
  local
    ...
    and injectTyConToTop
          ({name,
            ...
            id,
            eqKind,
            constructorHasArgFlagList} : TY.tyCon)
      =
      (
       {
        name = name,
    ...
    and injectConPathInfoToTop
          {namePath = (name, path), funtyCon, ty, tag, tyCon} =
        {
         namePath = (name, injectPathToTop path),
         funtyCon = funtyCon,
         ty = injectTyToTop ty,
    ...
    and injectTyBindInfoToTop tyBindInfo =
        ...
        | TY.TYOPAQUE {spec, impl} =>
          TY.TYOPAQUE {spec = injectTyConToTop spec, impl = impl}
                ...
                (fn {funName, 
                    ...
                    let
                      fun stripSysPathNPEnv injectFun NPVarEnv = 
                          NPEnv.foldli
                            (fn (namePath, idState, newNPVarEnv) =>
                                NPEnv.insert
                                  (newNPVarEnv,
        ...
        in
          {
           ...
           tyConEnv = SEnv.unionWith
                        #1
                        (NPTyConEnvToSEnv newTyConEnv, topTyConEnv),
           sigEnv = SEnv.unionWith
                      #1
                      (newSigEnv, topSigEnv),
           funEnv = SEnv.unionWith
                      #1
                      (injectFunctorEnv newFunEnv, topFunEnv)
          }
        end
