      ...
      let
        fun makeClosureTy rawTy =
          let
            ...
            val kindedTvarList = map (fn tvar => (tvar, A.UNIV)) (SEnv.listItems tvarSEnv)
          in
         ...
         *)
        val context1 = 
            TC.bindVarInEmptyContext (T.toplevelDepth, namePath, T.VARID valDesc)
        val context2 =
  ...
  *)
  and typeinfTypdescs eqKind (basis : TIC.basis) nil loc = 
    ...
    | typeinfTypdescs eqKind (basis : TIC.basis) 
      ...
      (* rule 80-for rule 69 *)
      let
        val tyCon = {name = tyconName,
                     strpath = strpath,
                     abstract = false,
                     id = Counters.newTyConId (),
                     eqKind = ref eqKind,
                     tyvars = map (fn {name, eq} => if eq = Absyn.EQ then T.EQ else T.NONEQ) tyvars,
                     constructorHasArgFlagList = nil}
        val context1 = TC.bindTyConInEmptyContext ((tyconName, strpath), T.TYSPEC tyCon)
        ...
        val _ =
            ...
            | SOME tau' =>
              if  OTSet.isEmpty (TU.EFTV tau') then () 
                     ...
                     (loc, E.FreeTypeVariablesInExceptionType({exid = NM.namePathToString(namePath)}))
        val exnPathInfo = TIU.makeExnConPath namePath
                                             tau
        val context1 = TC.bindVarInEmptyContext (T.toplevelDepth, namePath, T.EXNID exnPathInfo)
  ...
  and typeinfSpec (basis:TIC.basis) ptspec =
      ...
      | PT.PTSPECTYPEEQUATION (tvarListStringRawTy as (_,tyConNamePath,_) ,loc) =>
        (*
         * sig type t = int end ==> sig include type t end where type t = int end
        ...
        (* appendix A *)
          let
            ...
            val newContext as {tyConEnv,...} =
                  typeinfSpec basis (expandManifestTypeDesc tvarListStringRawTy)
            val tyBindInfo = 
                case NPEnv.find(tyConEnv, tyConNamePath) of
                  NONE => 
                    raise Control.BugWithLoc (("unbound tyCon:"^NM.namePathToString(tyConNamePath)),
                                              loc)
                | SOME tyFun => tyFun
          in
              newContext
      ...
      | PT.PTSPECDATATYPE(prefix, datadescs, loc) => 
          ...
          let
            ...
            *)
            val (newContext, tyCons) = 
                  TIFC.typeinfDatatypeDecl T.toplevelDepth basis (prefix, newdatadescs) loc
          in
              newContext
          end
                ...
                let
                    val leftDataTyInfo : T.dataTyInfo =
                        {tyCon = {name = tyConName,
                         ...
                         datacon = SEnv.empty} 
                    val tyConSubst = TyConID.Map.singleton(id, T.TYCON leftDataTyInfo)
                    val newDatacon = 
		        TCU.substTyConInDataConFully tyConSubst datacon
                    val leftDataTyInfo =
                        {tyCon = #tyCon leftDataTyInfo, datacon = newDatacon}
                    val context1 = 
                        TC.bindTyConInEmptyContext((tyConName, tyConStrpath), T.TYCON leftDataTyInfo)
                    val context2 = 
                        (* do not propagate varEnv of abstype *)
                        if not abstract then
                            let
                                val (newDatacon, _) = 
                                    TIU.setPrefixDataCon ((#datacon leftDataTyInfo), tyConStrpath)
                            in
                                TC.extendContextWithVarEnv (context1, newDatacon)
                            end
                        else context1
                in
                    context2
         ...
         (let
              ...
              val (representativeTyConId, hdEqKind) = 
                  let
                      ...
                      val (rpTyConId, eqKind) = SU.longTyConIdEqKind newBasis hdLongTyCon
                  in
                  ...
                  List.foldr 
                      (fn (longTyCon, (tyConIdSubst, othersOverallEqKind)) =>
                          ...
                          in
                              ...
                              then
                                  (TyConID.Map.insert(tyConIdSubst, tyConId, representativeTyConId),
                                   if eqKind = T.EQ then eqKind else othersOverallEqKind)
                              else
                                  raise E.RigidTypeInSharing {id = NM.namePathToString(longTyCon)}
                          end)
              ...
              val newTyConIdEqSubst = 
                  if hdEqKind = T.EQ orelse othersOverallEqKind = T.EQ then
                      ...
                      TyConID.Map.map (fn newId => (newId, T.NONEQ)) 
                                       tyConIdEqSubst
              val newContext = SU.equateTyConIdEqInContext newTyConIdEqSubst context1
          in 
              newContext
          end
      ...
      | PT.PTSPECSIGWHERE (ptspec, stringListLongTyConTyList, loc) => 
        let
            ...
            val (tyConSubst, pathTyFunInfoList) =
                List.foldr
                    (fn ((typarams, longTyCon as (name, path), ty), (tyConIdEnv, tyFunInfoList)) =>
                        let
                          val (_, newTyFun) = TIFC.makeTyFun 
                                                  ...
                                                  (typarams, 
                                                   ...
                                                   ty)
                          val tyConId =
                              case TIC.lookupTyConInBasis (basis1, longTyCon) of
                                NONE => 
                                raise E.TyConNotFoundInWhereType
                                          {tyCon = NM.namePathToString(longTyCon)}
                              | SOME (T.TYCON {tyCon = {name,id,eqKind,tyvars,...}, ...})=> 
                                if (TyConID.compare(fromTyConId, id) <> LESS)
                                then
                                    raise E.RigidTypeInRealisation {id = NM.namePathToString(longTyCon)}
                                else
                              ...
                              | SOME (T.TYFUN tyFun) => 
                                ...
                                then
                                    ...
                                    in
                                        if (TyConID.compare(fromTyConId, #id tyCon) <> LESS)
                                        then
                                            raise E.RigidTypeInRealisation {id = NM.namePathToString(longTyCon)}
                                        else
                                            ...
                                            then
                                                ...
                                                if List.length (#tyvars tyCon) <> List.length typarams
                                                then
                                                    raise E.ArityMismatchInWhereType 
                                                              {
                              ...
                              | SOME (T.TYOPAQUE _) => 
                                raise E.TyFunWithWhereType
                                          {longTyCon = NM.namePathToString(longTyCon)}
                              | SOME(T.TYSPEC {name, strpath, id, eqKind, tyvars,...}) => 
                                 if (!eqKind = T.EQ) andalso
                                    not (TU.admitEqTyFun newTyFun)
                                 then 
                                     raise E.EqtypeRequiredInWhereType {
...
(*
      | PT.PTSPECFUNCTOR (fundescList, loc) =>
        let
            val functorEnv = 
                foldl (fn ((funName, (argSpec, argNamePathEnv), (bodySpec, bodyNamePathEnv)), functorEnv) =>
                          let
                              val sigma as (T,E) = typeinfSig basis argSpec
                              (* signature environment without argument 
                               * structure prefixing.
                               *)
                              val newArgE = TIU.constructEnvFromNameMap (E, argNamePathEnv) 
                              val newBasis = TIC.extendBasisWithBasicEnv (basis, E)
                              val sigma' as (T', E') = typeinfSig newBasis bodySpec
                              val newBodyE = TIU.constructEnvFromNameMap (E, bodyNamePathEnv) 
                          in
                              SEnv.insert(functorEnv,
                                          funName,
                                          { 
	                                   funName = funName, 
	                                   argName = "?X",
	                                   functorSig = 
                                           {
                                            generativeExnTagSet = TIU.exnTagSetEnv E',
		                            argTyConIdSet = T,
		                            argSigEnv = newArgE,
                                            argStrPrefixedEnv = E,
			                    body = sigma'
                      ...
                      fundescList
        in
            (TC.injectFunctorEnvToContext functorEnv)
        end
*)
    ...
    and typeinfFunbinds (basis:TIC.basis) nil = (TC.emptyContext, nil)
      | typeinfFunbinds (basis:TIC.basis) (ptfunbind as (funName, 
                                                         (argSpec, argName, 
    ...
    and typeinfPtCoreDecs (basis : TIC.basis) plCoreDecs =
        let
            val (newBasis, context, newTpCoreDecs) =
                foldl (fn (plCoreDec, (basis, context, newTpCoreDecs)) =>
                          let
                              val (context1, decs) =
        ...
        case plStrDec of
            ...
            let
                val (context, newDecUnits) =
                    foldl (fn ((*printSigInfo is for printing in printCodeGeneration *)
          ...
          | PT.PTTRANCONSTRAINT(ptdecs, richFlattenedNameMap, ptspec, strictFlattenedNameMap, loc) =>
            (* rule 52 *)
            (* For example, 
             *  structure S =
             *     struct
            ...
            let
                ...
                val flattenedSysStrictEnv = 
                    ...
                    TIU.restoreSysNamePathSigEnv (flattenedSrcStrictEnv, richFlattenedNameMap) loc
                val context3 = TC.injectEnvToContext flattenedSysStrictEnv
            in
                (context3, 
                 [TCC.TPCONSTRAINT(newDecs @ instantiatedDecs, strictFlattenedNameMap, loc)])
            end
          | PT.PTOPAQCONSTRAINT(ptdecs, richFlattenedNameMap, ptspec, strictFlattenedNameMap, loc) =>
            (* rule 53 *)
            let
                val (context1, newDecs) = typeinfPtStrDecs basis ptdecs
                ...
                val context3 = 
                    TC.injectEnvToContext 
                        (TIU.restoreSysNamePathSigEnv (abstractEnv, richFlattenedNameMap) loc)
            in
                (context3, 
                 [TCC.TPCONSTRAINT(newDecs @ instantiatedDecs, strictFlattenedNameMap, loc)])
            end
          | PT.PTFUNCTORAPP (prefix, 
                ...
                val funBindInfo as {functorSig, ...}
                  ...
                  | NONE =>
                    (E.enqueueError (loc, E.FunctorNotFound{id = functorName});
                     ...
                     :T.funBindInfo)
                val Env = 
                    TIU.constructActualArgTypeEnvFromNameMap actualFlattenedNamePathEnv basis loc
                val (resEnv, argMatchedEnv, generativeExnTagTable, exnTagResolutionTable) =
                    let
                        val defaultErrorValue = 
                            (T.emptyE, T.emptyE, ExnTagID.Map.empty, ExnTagID.Map.empty)
                    in
                        ( 
            ...
            in
                (context2, 
                 [TCC.TPSTRLOCAL(instantiatedDecs, 
                                 [TCC.TPFUNCTORAPP 
                                      {prefix = prefix,
                                       funBindInfo =  funBindInfo,
                                       argNameMapInfo = {argNamePath = actualArgNamePath, 
                ...
                case pltopdec of
                    ...
                    let
                        val (newContext, _, newTpStrDecs) =
                            foldl (fn (ptStrDec, (context, newBasis, ptStrDecs)) =>
                                      let
