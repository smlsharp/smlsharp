...
in
    ...
    fun longTyConIdEqKind basis longTyCon = 
        case TIC.lookupTyConInBasis (basis, longTyCon) of
            NONE => raise E.TyConNotFoundInShare ({tyCon = NameMap.namePathToString(longTyCon)})
          | SOME(T.TYCON {tyCon = {id, eqKind, ...}, ...}) => (id, !eqKind)
          | SOME(T.TYOPAQUE({spec = {id, eqKind, ...}, ...})) => (id, !eqKind)
          | SOME(T.TYSPEC({id, eqKind, ...})) => (id, !eqKind)
          | SOME(T.TYFUN({name,...})) => raise (E.SharingOnTypeFun {tyConName = name})

    fun extendTyConIdSubst (newSubst, oldSubst) =
        let
            val revisedOldSubst = TyConID.Map.map (TCU.substTyConIdInId newSubst) oldSubst
        in
    ...
    fun equateTyConIdEqInContext tyConIdEqSubst ({tyConEnv, varEnv, sigEnv, funEnv}:TC.context) =
        {
    ...
    fun instExnTagBySubstOnVarEnv subst (varEnv:T.varEnv) =
        NPEnv.map ( fn idstate:T.idState =>
                       case idstate of
                ...
                case TyConID.Map.find(tyConSubst, #id tyCon) of
                  ...
                  | SOME tyBindInfo => 
                    case (TU.peelTyOPAQUE tyBindInfo) of
                      ...
                      | T.TYOPAQUE _ => raise Control.Bug "TYOPAQUEty occurs after peelTyOPAQUE"
        in
            newTy
        end
    ...
    and instOPAQUEty strPathSubst tyConSubst ty =
        case ty of
          ...
          | _ => raise Control.Bug "non ABSSPECty to instABSSPECty (typeinference/main/sigutils.sml)"
        ...
        TypeTransducer.mapTyPreOrder
            (fn ty =>
                case ty of
                    ...
                    in (ty, true) end
                  | T.RAWty {tyCon, args} => 
                    (T.RAWty {tyCon = instSigTyConStrPath strPathSubst tyCon, args = args}, 
                     true) 
                  | T.OPAQUEty {spec = {tyCon, args}, implTy} =>
                    (T.OPAQUEty {spec = {tyCon = instSigTyConStrPath strPathSubst tyCon,
                                         args = args},
                                 implTy = implTy},
                     true)
                  | T.SPECty spec =>
    ...
    and instSigTvKind strPathSubst tyConSubst {lambdaDepth, id, recordKind, eqKind, tyvarName} = 
        let
            val recordKind =
                case recordKind of 
                    T.UNIV => T.UNIV
    ...
    and instSigTyConStrPath strPathSubst (tyCon as {name, strpath, abstract, tyvars, id, eqKind, constructorHasArgFlagList}) =
      let 
          val strpath = 
    ...
    and instSigIdstate strPathSubst tyConSubst idstate =
        case idstate of
          ...
          | T.RECFUNID ({namePath, ty}, int) => 
            T.RECFUNID ({namePath = namePath, ty = instSigTy strPathSubst tyConSubst ty}, int)
          | T.PRIM x => T.PRIM x
          | T.OPRIM x => T.OPRIM x
    ...
    and dataTyInfoToAbsTyCon  {tyCon, datacon} : Types.dataTyInfo =
        let
            ...
            val tyConSubst = 
                TyConID.Map.singleton(#id tyCon, 
     ...
     *)
    and instTopSigEnv (Env as (tyConEnv2, varEnv2), sigEnv as (tyConEnv1, varEnv1)) = 
        let
                        ...
                        case tyBindInfo1 of
                            ...
                            (case NPEnv.find(tyConEnv2,tyConNamePath) of
                               ...
                               | SOME tyBindInfo => 
                                 let
                                     val strPathSubst =  
                                         strPathEnv.insert(strPathSubst, (id,strpath), TU.strPathOfTyBindInfo tyBindInfo)
                                     val tyConSubst = 
                                         case tyBindInfo of
                                             T.TYCON dataTyInfo => 
                                             TyConID.Map.insert
                                                 (tyConSubst, id, T.TYCON(dataTyInfoToAbsTyCon dataTyInfo))
                                           | _ => TyConID.Map.insert(tyConSubst, id, tyBindInfo) 
                                 in
                                     (strPathSubst, tyConSubst)
                                 end)
                          ...
                          | T.TYFUN {name, strpath, tyargs, body = T.ALIASty(T.RAWty {tyCon = {id,...},...},ty)}=> 
                            (case NPEnv.find(tyConEnv2, tyConNamePath) of
                                 NONE => (* error case captured by signature check *)
                                 (strPathSubst, tyConSubst)
                               | SOME tyBindInfo =>
