...
struct
  ...
  fun checkTy (arg as (msg, loc)) (env:T.btvEnv) ty =
      ...
      | T.RECORDty {fieldTypes, annotation} =>
        app (checkTy arg env) (SEnv.listItems fieldTypes)
  ...
  and checkTvarKind arg env kind =
      ...
      | T.REC fields => app (checkTy arg env) (SEnv.listItems fields)
  ...
  and unifyRecordFields btv (fields1, fields2) =
      let
        val (labels1, tys1) = ListPair.unzip (SEnv.listItemsi fields1)
        val (labels2, tys2) = ListPair.unzip (SEnv.listItemsi fields2)
  ...
  fun typecheckConstTerm const =
      ...
      | ConstantTerm.NULLPOINTER =>
        T.CONty {tyCon = BuiltinEnv.PTRtyCon, args = [T.unitty]}
      | ConstantTerm.NULLBOXED =>
        T.CONty {tyCon = BuiltinEnv.BOXEDtyCon, args = []}
