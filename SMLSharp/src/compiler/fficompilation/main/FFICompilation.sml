...
struct
  ...
  fun implodeRecord (fields, loc) =
      let
        val tys = SEnv.fromList (map (fn (l,{ty,exp}) => (l,ty)) fields)
        val exps = SEnv.fromList (map (fn (l,{ty,exp}) => (l,exp)) fields)
  ...
  fun composeArg (nil, loc) =
      {ty = BuiltinEnv.UNITty,
       exp = R.RCCONSTANT {const=A.UNITCONST loc, ty=BuiltinEnv.UNITty,
                           loc=loc}}
    | composeArg ([exp], loc) = exp
    | composeArg (exps, loc) = implodeRecord (tupleLabels exps, loc)

  fun decomposeArg (nil, loc) = (newVar BuiltinEnv.UNITty, nil)
    | decomposeArg ([ty], loc) =
      let val var = newVar ty
      in (var, [varExp loc var])
      end
    | decomposeArg (tys, loc) =
      let val var = newVar (T.RECORDty (SEnv.fromList (tupleLabels tys)))
  ...
  and stubExport ffity =
      ...
      | TC.FFIRECORDTY (fields, loc) =>
        ...
        in
          (T.RECORDty (SEnv.fromList retTys),
  ...
  fun compileImport (ptrExp, ffiTy, resultTy, loc) =
      let
        ...
        val var = newVar BuiltinEnv.PTRty
        val (_, importFn) = stubImport ffiTy
        fun stub exp = #exp (importFn {ty = BuiltinEnv.PTRty, exp = exp})
      in
        if isSimpleExp exp
