...
struct
  ...
  fun transformPrim env baprim =
      ...
      | C.BARECORD {fieldList, recordTy, annotation,
                    isMutable, totalSizeExp, bitmapExpList} =>
        let
...
*)
  ...
  fun transformTopdec batopdec =
      case batopdec of
        C.CAFUNCTION {codeId, path, btvEnv, bodyTy, attributes, closureEnvArg,
                      argVarList, frameBitmapExp, frameBitmaps,
                      frameBitmapBits,
                      outerFrameBitmap=(freeTyvars, freeBitmaps),
        ...
        let
          ...
          val (freeTyvars, freeBitmaps) =
              case freeTids of
                nil => (nil, nil)
              | _::_ => (freeTyvars, freeBitmaps)
          val tyvars = boundTyvars @ freeTyvars
          val (env, argVarList) = bindVarList env A.ARG argVarList
          ...
          val bitmapFree = transformBitmapFree (body, freeBitmaps)
                           handle e => raise e
        in
      ...
      | C.CARECFUNCTION {closureEnvArg, frameBitmapExp, frameBitmaps,
                         frameBitmapBits, functions, loc} =>
        let
          val env = newEnv closureEnvArg
          local
            val mergeLabel = VarID.generate ()
