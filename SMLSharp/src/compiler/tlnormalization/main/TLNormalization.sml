...
struct
  ...
  fun makeArrayType elementTy = T.RAWty{tyCon = PT.arrayTyCon, args = [elementTy]}
  ...
  fun makeRefValIndex loc = TLCONSTANT{value = CT.INT(Int32.fromInt 0), loc = loc}
  ...
  fun makePrimApply (primOp:RC.primInfo, instTyList, argExpList, loc) =
      ...
      | (P.S P.Array_update_unsafe, _, _) => raise Control.Bug "Array_update_unsafe"
      ...
      | (P.P prim, _, _) => 
        TLPRIMAPPLY 
            {
             primInfo = transformPrimOp ({name = prim, ty = #ty primOp}, instTyList), 
      ...
      let
        val argTy = 
          ...
          | _ => 
            raise 
            Control.Bug 
            "non FUNMty in constructionLayout (tlnormalization/main/TLNormalization.sml)"
      in
        if isFlattened ty 
  ...
  fun transformExp vEnv rexp =
      ...
      | RC.RCGLOBALSYMBOL (name, kind, ty, loc) => TLGLOBALSYMBOL {name=name,kind=kind,ty=ty,loc=loc}
         ...
         case VarEnv.find(vEnv, varInfo) of
           ...
           in
             TLRECORD
                 {
                  isMutable = false,
                  expList = map makeFieldExp (SEnv.listItemsi (fieldTypes argRecordTy)),
        ...
        TLGETFIELD
            {
             ...
             indexExp = TLCONSTANT {value = CT.INT (Int32.fromInt index), loc = loc},
             elementTy = elementTy,
             loc = loc
            }
      ...
      | RC.RCPRIMAPPLY {primOp as {ty,...}, instTyList, argExpOpt = SOME argExp, loc} =>
         ...
         *)
        let
          val (argLabelTyList, domTy, ranTy, newFunTy) = expandFunTy (TU.tpappTy (ty, instTyList))
        in
          case argLabelTyList of
            [] => makePrimApply (primOp, instTyList, [makeUnitVal loc], loc )
          | [(l,_)] => makePrimApply(primOp, instTyList, [transformExp vEnv argExp],loc)
          | _ =>
            let
      ...
      | RC.RCDATACONSTRUCT {con as {funtyCon = true, ...}, argExpOpt = NONE, ...} =>
        raise Control.Bug "funtycon but no args"
      | RC.RCEXNCONSTRUCT {exn as {funtyCon = true, ...}, argExpOpt = NONE, ...} =>
        raise Control.Bug "funtycon but no args"
      | RC.RCDATACONSTRUCT {con as {ty, ...},  instTyList, argExpOpt=NONE, loc} =>
        let
...
*)
      | RC.RCEXNCONSTRUCT {exn as {ty, ...},  instTyList, argExpOpt=NONE, loc} =>
        let
          val externalTy = TU.derefTy (TU.tpappTy(ty, instTyList))
        in
            makeConstruct
...
*)
      | RC.RCDATACONSTRUCT {con as {funtyCon = false, ...}, argExpOpt = SOME _,...} =>
        raise Control.Bug "nonfuntycon with args"
      | RC.RCEXNCONSTRUCT {exn as {funtyCon = false, ...}, argExpOpt = SOME _,...} =>
        let
            val _  = print (Control.prettyPrint (RecordCalc.format_rcexp nil rexp))
        in
            raise Control.Bug "nonfuntycon with args"
        end
      | RC.RCDATACONSTRUCT {con as {ty, tyCon, ...}, instTyList, argExpOpt = SOME argExp, loc} =>
        let 
          val externalTy = 
              case TU.derefTy (TU.tpappTy(ty, instTyList)) of
              ...
              TLCAST
                  {
                   exp = TLARRAY 
                             {
                              sizeExp = TLCONSTANT{value = CT.INT (Int32.fromInt 1), loc = loc}, 
              ...
              val newExp =
                    ...
                    (
                     case argExp of 
                       ...
                       TLRECORD
                       {
                        isMutable = false,
                        expList = 
                          (makeTagExpForDataCon con loc)::(map (transformExp vEnv) (SEnv.listItems fields)),
                       ...
                       let
                         ...
                         fun K var =
                             ...
                             in
                               ...
                               {
                                isMutable = false,
                                expList = (makeTagExpForDataCon con loc)::argExpList,
                                recordTy = makeFlatConstructType tagty argTy,
                                loc = loc
                                }
                             end
                  ...
                  | NESTED_CONSTRUCT argTy =>
                    TLRECORD
                        {
                         isMutable = false,
                         expList = [makeTagExpForDataCon con loc, transformExp vEnv argExp],
      ...
      | RC.RCEXNCONSTRUCT {exn as {ty, tyCon, ...}, instTyList, argExpOpt = SOME argExp, loc} =>
        let 
          val externalTy = 
              case TU.derefTy (TU.tpappTy(ty, instTyList)) of
                T.FUNMty (argTyList, bodyTy) => bodyTy
              ...
              val newExp =
                    ...
                    (
                     case argExp of 
                       ...
                       TLRECORD
                           {
                            isMutable = false,
                            expList = (makeTagExpForExnCon exn loc)::(map (transformExp vEnv) (SEnv.listItems fields)),
                         ...
                         fun K var =
                             ...
                             in
                               TLRECORD
                                   {
                                    isMutable = false,
                                    expList = (makeTagExpForExnCon exn loc)::argExpList,
                                    recordTy = makeFlatConstructType PT.exntagty argTy,
                                    loc = loc
                                   }
                             end
                  ...
                  | NESTED_CONSTRUCT argTy =>
                    TLRECORD
                        {
                         isMutable = false,
                         expList = [makeTagExpForExnCon exn loc, transformExp vEnv argExp],
      ...
      | RC.RCCASE {exp, expTy, ruleList, defaultExp, loc} =>
        ...
        then
          case (ruleList, TU.derefTy expTy) of
            ([(c, SOME boundVar, mainExp)],  T.RAWty{tyCon, args = [elementTy]}) =>
            let 
              val boundExp =
                  TLGETFIELD
                      {
              ...
              let
                ...
                fun processRule (con, SOME (varInfo as {ty = argTy,...}), body) = 
                  (
                   ...
                   | NESTED_CONSTRUCT _ =>
                       let
                         val nestedConstructTy = makeNestedConstructType tagty argTy
                         val boundExp =
                           TLSELECT
                           {
                            recordExp = TLCAST{exp = selector, 
                 ...
                 fun K selector =
                   let
                     ...
                     val switchExp = 
                           TLSELECT 
                           {
                            recordExp = TLCAST{exp = selector, targetTy = exntagRecordType, loc = loc},
                            label = CONTAG_LABEL,
                            recordTy = exntagRecordType,
                     ...
                     fun processRule (con, SOME (varInfo as {ty = argTy,...}), body) = 
                       (
                        case constructionLayout (#ty con,tyArgList) of
                          FLAT_CONSTRUCT _ =>
                            let
                              val flatConstructTy = makeFlatConstructType PT.exntagty argTy
                              val varEnvEntry =
                                {
                                 argRecordTy = argTy,
                                 flatConstructTy = flatConstructTy,
                        ...
                        | NESTED_CONSTRUCT _ =>
                            let
                              val nestedConstructTy = makeNestedConstructType PT.exntagty argTy
                              val boundExp =
                                TLSELECT
                                {
                                 recordExp = TLCAST{exp = selector, 
...
(*
         ...
         then
             (* Liu :in separate/functor compilation mode, if boxedKind = GENERICty 
              * then adopt this case ?
...
*)
            ...
            makeConstruct 
                (
                 TLRECORD 
                     {
                      isMutable = false,
                      expList = [makeTagExpForDataCon (T.conPathInfoToConInfo PT.nilConPathInfo) loc], 
                      recordTy = tagRecordType,
                      loc = loc
                     },
                 listTy,
          ...
          fun makeNilCons consExp = 
            let
              ...
              val newExp = 
                ...
                {
                 isMutable = true,
                 expList = [makeTagExpForDataCon (T.conPathInfoToConInfo PT.consConPathInfo) loc, consExp, makeNil loc],
        ...
        let 
          ...
          val boundExp =
              TLSETFIELD
                  {
                   arrayExp = transformExp vEnv arrayExp,
                   indexExp = TLCONSTANT{value = CT.INT (Int32.fromInt indexExp), loc = loc},
  ...
  fun transformTopBlock vEnv topBlock = 
      case topBlock of
          RC.RCBASICBLOCK basicBlock => TLBASICBLOCK (transformBasicBlock vEnv basicBlock)
        | RC.RCFUNCTORBLOCK {name, formalAbstractTypeIDSet, formalVarIDSet, formalExnIDSet,
