...
in
  ...
  and uncurryExp spine tpexp = 
    ...
    | TC.TPLET {decls, body, tys, loc} =>
      let
        val decls = uncurryDeclList  false decls
  ...
  and uncurryDecl isTop tpdecl = 
      ...
      | TC.TPPOLYFUNDECL (btvEnv, fundecls, loc) =>
        if isTop then
                ...
                [TC.TPVALPOLYREC
                   (btvEnv, 
                    map (fn (v,ty,exp) => {exp=exp, expTy = ty, var = v})
                        (map (matchToFnCaseTerm  loc) fundecls), 
                    loc)]
            fun instantiateAndCurryFun {funVarInfo = {path,id,ty}, 
                                        argTyList, 
                                        bodyTy, ruleList} =
                if length argTyList = 1 then
                  let
                    ...
                    val polyFunTerm =
                        TC.TPVAR({path=path, id=id, ty = polyFunTy},loc)
                  in
                    ({path=path,id=id,ty=polyFunTy}, polyFunTerm)
                  end
                else
                  let
                    val subst = TypesUtils.freshSubst  btvEnv
                    val instTyList = BoundTypeVarID.Map.listItems subst
                    val instArgTyList =
                    ...
                    val polyFunTy =
                        T.POLYty{boundtvars=newBtvEnv, body = unCurriedFunTy}
                    val polyFunTerm =
                        TC.TPVAR({path=path,id=id,ty=polyFunTy},loc)
                    val unCurriedFunVar = 
                        TC.TPTAPP{exp = polyFunTerm,
                                  expTy = polyFunTy,
                                  instTyList = instTyList,
                                  loc = loc}
                    val newVars = map makeVar instArgTyList
                    val newBodyTerm = 
                        TC.TPAPPM
                          {argExpList = map (fn v => TC.TPVAR(v,loc)) newVars,
                  ...
                  in
                    (
                     {path=path,
                      id=id,
                      ty=T.POLYty{boundtvars=newBtvEnv,body=curriedFunTy}},
                     TC.TPPOLY {btvEnv=newBtvEnv, 
          ...
          in
            polydecls @ [TC.TPVAL(outerBinds,loc)]
          end
        else [TC.TPVALPOLYREC
                (btvEnv, 
  ...
  and optimize decList =  uncurryDeclList true decList handle exn => raise exn
end
end
