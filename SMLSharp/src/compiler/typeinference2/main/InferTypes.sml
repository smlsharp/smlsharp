...
local 
  ...
  fun printIcexp icexp =
      print (Control.prettyPrint (I.format_icexp icexp))
  ...
  fun printIcdecl icdecl =
      print (Control.prettyPrint (I.format_icdecl icdecl))
  fun printITy ty = 
      print (Control.prettyPrint (IT.format_ty ty))
  ...
  fun makeTupleTy nil = BE.UNITty
    | makeTupleTy [ty] = ty
    | makeTupleTy tys = T.RECORDty (SEnv.fromList (makeTupleFields tys))
  ...
  fun makeTupleExp (nil, loc) =
    ...
    | makeTupleExp (fields, loc) =
      ...
      in
        TC.TPRECORD {fields = SEnv.fromList (makeTupleFields exps),
                     recordTy = makeTupleTy tys,
                     loc = loc}
      end

  fun SEnv_all f env =
      SEnv.foldl (fn (x,z) => z andalso f x) true env
  ...
  fun getRuleLocM nil = raise bug "empty rule in getRuleLocM"
    | getRuleLocM [{args=pat::_, body}] =
        Loc.mergeLocs (I.getLocPat pat, I.getLocExp body)
    | getRuleLocM rules =
        let
          val pat1 = 
            case rules of
        ...
        in
          Loc.mergeLocs (I.getLocPat pat1, I.getLocExp body)
        end
      ...
      let
        ...
        fun tySubst ty = 
            ...
            | T.RECORDty tySenvMap =>
              T.RECORDty (SEnv.map tySubst tySenvMap)
            | T.CONSTRUCTty {tyCon:T.tyCon as {path,...},args} =>
              (case TypID.Map.find(typIdMap, #id tyCon) of
               ...
               | SOME ({id,iseq,arity,conSet,extraArgs,dtyKind,...}:T.tyCon) =>
                 let
                   val tyCon =
                       {id=id,
                        iseq=iseq,
        ...
        and tvarKindSubst tvarKind =
            ...
            | T.UNIV => T.UNIV
            | T.REC tySenvMap =>
              T.REC (SEnv.map tySubst tySenvMap)
      in
        tySubst ty
      end
      ...
      let
        ...
        fun expSubst tpexp = 
            ...
            | TC.TPRECORD {fields, recordTy, loc} =>
              TC.TPRECORD
                {fields = SEnv.map expSubst fields,
        ...
        and tvarKindSubst {eqKind, tvarKind} =
            {eqKind=eqKind,
             ...
             | T.UNIV => T.UNIV
             | T.REC tyMap => T.REC (SEnv.map tySubst tyMap)
            }
        and patSubst pat =
            case pat of
            ...
            | TC.TPPATRECORD {fields, recordTy, loc} =>
              TC.TPPATRECORD
                {fields = SEnv.map patSubst fields,
                 recordTy = tySubst recordTy,
                 loc = loc
                }
        ...
        and declSubst decl =
          ...
          | TC.TPEXPORTVAR (var, loc) =>
            TC.TPEXPORTVAR (varSubst var, loc)
...
in
  ...
  and isInteroperableTycon dir ({id, dtyKind, ...}:T.tyCon, args) =
      case dtyKind of
        T.BUILTIN BuiltinType.INTty => true
      | T.BUILTIN BuiltinType.WORDty => true
      | T.BUILTIN BuiltinType.WORD8ty => true
      | T.BUILTIN BuiltinType.CHARty => true
      | T.BUILTIN BuiltinType.STRINGty => exportOnly dir
      | T.BUILTIN BuiltinType.REALty => true
      | T.BUILTIN BuiltinType.REAL32ty => true
      | T.BUILTIN BuiltinType.UNITty => false
      | T.BUILTIN BuiltinType.PTRty =>
      ...
      | T.BUILTIN BuiltinType.EXNty => false
      | T.BUILTIN BuiltinType.INTINFty => exportOnly dir
      | T.BUILTIN BuiltinType.BOXEDty => false
      | T.BUILTIN BuiltinType.ARRAYty =>
        exportOnly dir andalso List.all (isInteroperableArgTy dir) args
      | T.BUILTIN BuiltinType.EXNTAGty => false
  ...
  and isInteroperableTy dir ty =
      case TU.derefTy ty of
        ...
        isInteroperableTycon dir (tyCon, args)
      | T.RECORDty fields =>
        exportOnly dir andalso SEnv_all (isInteroperableArgTy dir) fields
      | _ => false

  fun evalForceImportFFIty (context:TIC.context) ffity =
      case ffity of
        IT.FFIBASETY (ty, loc) =>
        (Ety.evalIty context ty
         handle e => raise e)
      | IT.FFIFUNTY (_, _, _, loc) =>
        (E.enqueueError "Typeinf 001" (loc, E.ForceImportForeignFunction("001", ffity));
         T.ERRORty)
      | IT.FFIRECORDTY (fields, loc) =>
        T.RECORDty
          (SEnv.fromList
  ...
  fun evalFFIFunTyArgs (context:TIC.context) dir ffitys =
      case ffitys of
        ...
        (
          ...
          case ty of
            IT.TYCONSTRUCT
              {typ={tfun=IT.TFUN_VAR(ref(IT.TFUN_DTY{id,...})),...},
               args=[]} =>
  ...
  and evalFFIty (context:TIC.context) dir ffity =
      case ffity of
        IT.FFIFUNTY (attributes, argTys, retTys, loc) =>
        let
  ...
  fun ffiStubTy ffity =
      ...
      | TC.FFIRECORDTY (fields, loc) =>
        T.RECORDty (SEnv.fromList (map (fn (k,v) => (k, ffiStubTy v)) fields))
  ...
  fun evalTvarKind (context:TIC.context) tvarkind =
    case tvarkind of
      IT.UNIV => T.UNIV
    | IT.REC fields => 
      T.REC (SEnv.map (Ety.evalIty context) fields)
      handle e => raise e
  ...
  fun isVar icexp = 
      case icexp of
        I.ICVAR _ => true
      | I.ICEXVAR _ => true
      | I.ICBUILTINVAR _ => true
      | I.ICCON _ => true
      | I.ICEXN _ => true
      | I.ICEXN_CONSTRUCTOR _ => true
      | I.ICEXEXN _ => true
      | I.ICOPRIM _ => true
      | I.ICTYPED  (icexp, ty, loc) => isVar icexp
      | I.ICSIGTYPED  {path, icexp, ty, loc, revealKey} => isVar icexp
      | _ => false
  ...
  fun stripIty icexp = 
      let 
        fun strip icexp ityList = 
          case icexp of
            I.ICTYPED (icexp, ity, _) => strip icexp (ity :: ityList)
          | I.ICSIGTYPED {icexp, ty, ...} => strip icexp (ty :: ityList)
          | _ => (icexp, ityList)
      in
        strip icexp nil
      end
  ...
  fun freeVarsInPat icpat =
      case icpat of
        I.ICPATERROR loc => VarSet.empty
      | I.ICPATWILD loc => VarSet.empty
      | I.ICPATVAR (varInfo, loc) => VarSet.singleton varInfo
      | I.ICPATCON (conInfo, loc) => VarSet.empty
      | I.ICPATEXN (exnInfo, loc) => VarSet.empty
      | I.ICPATEXEXN ({path, ty}, loc) => VarSet.empty
      | I.ICPATCONSTANT (constant, loc) => VarSet.empty
      | I.ICPATCONSTRUCT {con=icpat1, arg=icpat2, loc} => freeVarsInPat icpat2
      ...
      | I.ICPATTYPED (icpat, ty, loc) => freeVarsInPat icpat
  ...
  fun transFunDecl
      ...
      let
        val funBody = 
          let
            val newVars = map (fn _ => I.newICVar ()) patList
            val newVarExps = map (fn var => I.ICVAR(var, loc)) newVars
            val newVarPats = map (fn var => I.ICPATVAR(var, loc)) newVars
            val argRecord = I.ICRECORD (Utils.listToTuple newVarExps, loc)
            val funRules =
              map
              (fn {args, body} =>
               {args=[I.ICPATRECORD{flex=false,
          ...
          in
            foldr
            (fn (x, y) => I.ICFNM([{args=[x], body=y}], loc))
            (I.ICAPPM
             (
              I.ICFNM(funRules, loc),
              [argRecord],
              loc
              ))
            newVarPats
      ...
      in
        [(I.ICPATVAR (funVarInfo,loc), funBody)]
      end
 ...
 (* type generalization *)
  ...
  fun revealTy key ty =
      ...
      | T.RECORDty tyMap => T.RECORDty (SEnv.map (revealTy key) tyMap)
      | T.CONSTRUCTty
...
*)
      ...
      fun isStrictValuePat icpat =
          case icpat of
            I.ICPATERROR _ => false
          | I.ICPATWILD _ => true
          | I.ICPATVAR (varInfo, loc) => true
          | I.ICPATCON (conInfo, loc) => false 
          | I.ICPATEXN (exnInfo, loc) => false
          | I.ICPATEXEXN ({path, ty}, loc) => false
          | I.ICPATCONSTANT (constant, loc) => false
          | I.ICPATCONSTRUCT {con=icpat1, arg=icpat2, loc} => false
          | I.ICPATRECORD {flex, fields, loc} => 
            foldr
              (fn ((_, icpat), bool) =>
          ...
          | I.ICPATTYPED (icpat, ty, loc) => isStrictValuePat icpat
      ...
      fun decompose
            ...
            (icpat, icexp) = 
        let
          val icpatLoc = I.getLocPat icpat
          val icexpLoc = I.getLocExp icexp
              ...
              if VarSet.isEmpty varSet
                then
                  let
                    val newIcexp =
                      ...
                      (
                       [icexp], 
                       [{args=[icpat], body=I.ICRECORD(nil, icpatLoc)}],
                       PatternCalc.BIND,
                       loc
                       )
                    val (ty, tpexp) =
                  ...
                  [x as {path, id}] =>
                    let
                      val newIcexp =
                        ...
                        (
                         [icexp], 
                         [{args=[icpat], body=I.ICVAR(x, icpatLoc)}],
                         PatternCalc.BIND,
                         loc
                         )
                      val (ty, tpexp) =
                ...
                | _ =>
                  let
                    val resTuple =
                      makeTupleFields
                        (map (fn x => I.ICVAR (x, icpatLoc))
                             (VarSet.listItems varSet))
                    val newIcexp =
                      I.ICCASEM
                      (
                       [icexp], 
                       [{args=[icpat], body=I.ICRECORD (resTuple, icpatLoc)}],
                       PatternCalc.BIND,
                       loc
                       )
                    val (tupleTy, tpexp) =
        ...
        in (* decompose body *)
          if not (isStrictValuePat icpat) then makeCase (icpat, icexp)
            ...
            case icpat of 
              I.ICPATERROR loc => raise bug "expansive pat"
            | I.ICPATWILD loc =>
                let
                  val (ty, tpexp) =
...
*)
            ...
            | I.ICPATCON _ => raise bug "expansive pat"
            | I.ICPATEXN _ => raise bug "expansive pat"
            | I.ICPATEXEXN _ => raise bug "expansive pat"
            | I.ICPATCONSTANT _ => raise bug "expansive pat"
            | I.ICPATCONSTRUCT _ => raise bug "expansive pat"
            | I.ICPATRECORD {flex, fields=stringIcpatList, loc=loc1} =>
              (case icexp of
                 I.ICRECORD(stringIcexpList, loc2) =>
                 let 
                   val icpatSEnvMap = 
                       foldl
               ...
               | _ =>
                 let
                   ...
                   val (bodyVar as {path, id}) = I.newICVar()
                   val icBodyVar = I.ICVAR (bodyVar, loc1)
                       ...
                       map
                         (fn (label, icpat) =>
                             (
                              label,
                              icpat,
                              I.ICSELECT(label, icBodyVar, loc1)
                             ))
                         stringIcpatList
                   val (
                        localBinds,
            ...
            | I.ICPATLAYERED {patVar={path, id}, tyOpt, pat, loc} =>
              let
                val icexp =
                    case tyOpt of 
                      SOME rawty => I.ICTYPED (icexp, rawty, icexpLoc)
                    | NONE => icexp
            ...
            | I.ICPATTYPED (icpat, ity, loc) =>
              let 
                val icexp = I.ICTYPED (icexp, ity, icexpLoc)
              in
  ...
  and typeinfExp lambdaDepth applyDepth (context : TIC.context) icexp =
      ...
      | I.ICVAR (var as {path, id}, loc) =>
        (
         case VarMap.find(#varEnv context, var)  of
           SOME (TIC.VARID varInfo) => (#ty varInfo, TC.TPVAR (varInfo, loc))
         | SOME (TIC.RECFUNID (varInfo as {ty,...}, arity)) => 
      ...
      | I.ICEXN_CONSTRUCTOR (exn as {path, id, ty}, loc) =>
        let
          val ty = Ety.evalIty context ty
              handle e => raise e
          val exnInfo = {path=path, ty=ty, id=id}
      ...
      | I.ICTYPED (icexp, ty, loc) =>
         let
           ...
           val ty2 = Ety.evalIty context ty handle e => raise e
      ...
      | I.ICSIGTYPED {path, icexp=exp, ty, loc, revealKey} =>
         let
(*
...
*)
               ...
               (
                ...
                (
                 ...
                 E.SignatureMismatch ("012",{path=path, ty=instTy,
                                             annotatedTy=revealedTy2})
                 );
                (T.ERRORty, TC.TPERROR)
                )
         end
      | I.ICAPPM(I.ICRECORD_SELECTOR(l, loc1), [icexp], loc2) =>
         typeinfExp lambdaDepth applyDepth context (I.ICSELECT(l,icexp,loc2))
      ...
      | I.ICAPPM (icexp, icexpList, loc) =>
              ...
              let
                ...
                val _ =
                    app
                      (fn ity =>
                          let
                            val annotatedTy = Ety.evalIty context ity
                                handle e => raise e
                          in
          ...
          fun processCon (lambdaDepth,makeNewTermBody,funITy,funLoc) =
              case evalArgs lambdaDepth icexpList of
                ...
                let
                  ...
                  val polyFunTy = Ety.evalIty context funITy
                      handle e => raise e
                  val (funTy, instTyList) = TIU.freshTopLevelInstTy polyFunTy
                  val _ = 
                      app
                        (fn ity =>
                            let val annotatedTy1 = Ety.evalIty context ity
                                    handle e => raise e
                            in
               ...
               let
                 ...
                 val _ = 
                     app
                       (fn ity =>
                           let
                             val annotatedTy1 = Ety.evalIty context ity
                                 handle e => raise e
                           in
                             U.unify [(funTy, annotatedTy1)]
                             handle U.Unify =>
        ...
        in
          case funExp of 
            I.ICVAR (var, funVarLoc) =>
	    let
              val (funVar, arityOpt) = 
                  case VarMap.find(#varEnv context, var) of
                    SOME (TIC.VARID var) => (var, NONE)
                  | SOME (TIC.RECFUNID(var,arity)) => (var, SOME arity)
                  | NONE =>
                    (
(*
print "var not found in ICVAR\n";
NEU.printVarInfo var;
*)                    
                    raise bug "var not found"
                    )
	      val funExp = 
                  case arityOpt of
                    NONE => TC.TPVAR (funVar, funVarLoc)
                  | SOME arity =>  TC.TPRECFUNVAR{var=funVar,
                                                  arity=arity,
                                                  loc=funVarLoc}
              val funTy = #ty funVar
              val (funTy, funExp) = 
(*
                  TCU.freshInst (funTy, funExp)
*)
                  case funItyList of
                    nil => (funTy, funExp)
...
(*
          | I.ICBUILTINVAR {primitive, ty, loc} =>
            (case evalArgs lambdaDepth icexpList of
               ([argTy], [tpexp2]) =>
               let
                 val funTy = Ety.evalIty context ty
                 (*  a primitive type must be rank zero *)
...
*)
          | I.ICCON ({path, id, ty=funIty}, funLoc) =>
            let
...
*)
          fun eqList (tyList1, tyList2) =
              TIU.eqTyList BoundTypeVarID.Map.empty (tyList1, tyList2)
        in
          let
            val (funTy, funExp) =
                typeinfExp lambdaDepth (inc applyDepth) context icexp
      ...
      | I.ICLET (icdeclList, icexpList, loc) =>
        let 
          val (context1, tpdeclList) =
              typeinfDeclList lambdaDepth false context icdeclList 
          val newContext =
      ...
      | I.ICTYCAST (tycastList, icexp, loc) =>
        let
          ...
          val typIdMap =
              ...
              (fn ({from, to}, typIdMap) =>
                  let
                    val fromId = IT.tfunId from
                    val to = Ety.evalTfun context ["dummy"] to
                  in
...
*)
      ...
      | I.ICRECORD (stringIcexpList, loc) =>
        let 
      ...
      | I.ICHANDLE (icexp, icpatIcexpList, loc) =>
        let 
          val (ty1, tpexp) =
         ...
         | [{args=patList, body=exp}] =>
           ...
           in
             ...
             let
               fun getId icexp = 
                   let 
                     fun strip icexp ityList = 
                         case icexp of
                           I.ICPATVAR (var, _) => (var, ityList)
                         | I.ICPATWILD _ => (I.newICVar(), ityList)
                         | I.ICPATTYPED (icexp, ity, _) =>
               ...
               val newPtexp = I.ICFNM1(varList, exp, loc)
             in
             ...
             handle
               ...
               let
                 val varList =
                     map (fn pat => (I.newICVar (), I.getLocPat pat)) patList
                 val newPtexp =
                     I.ICFNM1
                       (
                        ...
                        I.ICCASEM
                          (
                           map (fn (var, loc) => I.ICVAR(var, loc)) varList,
                           argsBodyList,
                           PatternCalc.MATCH,
                           loc
                          ),
      ...
      | I.ICFNM1(varTyListList, icexp, loc) =>
         let 
           val lambdaDepth = incDepth ()
                 ...
                 (fn ((var as {path, id}, ityList),
                  ...
                  let 
                    ...
                    val _ = 
                        app
                          (fn ity =>
                              let val annotatedTy1 = Ety.evalIty newContext ity
                                      handle e => raise e
                              in
      ...
      | I.ICFNM1_POLY(varTyList, icexp, loc) =>
         let 
           val lambdaDepth = incDepth ()
           val (newContext, tyVarInfoList) =
               foldr
                 (fn ((var as {path, id}, ity),
                      (newContext, tyVarInfoList)) => 
                  let 
                    val domTy = Ety.evalIty newContext ity
                    val varInfo = {path=path, id=id, ty=domTy}
                    val newContext =
                        TIC.bindVar(lambdaDepth,
                                    newContext,
      ...
      | I.ICCASEM (icexpList, argsBodyList, caseKind, loc) =>
        let
          val (tyList, tpexpList) =
              foldr 
                (fn (ptexp, (tyList, tpexpList)) =>
      ...
      | I.ICRECORD_UPDATE (icexp, stringIcexpList, loc) =>
        let
          val (ty1, tpexp1) =
           ...
           | T.TYVARty (ref (T.TVAR tvkind)) =>
             let
               ...
               val recordTy =
                   ...
                   {
                    lambdaDepth = lambdaDepth,
                    tvarKind = T.REC (SEnv.singleton(label, elemTy)),
                    eqKind = A.NONEQ,
                    utvarOpt = NONE
                   }
             in
      ...
      | I.ICFFIEXPORT (icexp, ffity, loc) =>
        raise bug "ICFFIEXPORT"
      | I.ICFFIAPPLY (attributes, icexp, ffiArgList, ffiRetTy, loc) =>
        let
      ...
      | I.ICSQLSERVER (labeledExps, ty, loc) =>
        let
          val labeledExps =
              map
                (fn (label, icexp) =>
  ...
  and typeinfPat
      ...
      | I.ICPATVAR (var as {path, id}, loc) =>
        let
          val ty1 = T.newtyWithLambdaDepth (lambdaDepth, T.univKind)
          val varInfo = {path=path, id=id, ty=ty1}
          val varEnv1 = VarMap.insert (VarMap.empty, var, TIC.VARID varInfo)
        in
          (varEnv1, ty1, TC.TPPATVAR (varInfo, loc))
        end
      | I.ICPATCON ({path, id, ty=ity}, loc) =>
        let
          val ty = Ety.evalIty context ity
              handle e => raise e
      ...
      | I.ICPATCONSTRUCT {con=icpat1, arg=icpat2, loc} =>
        (case icpat1 of
           I.ICPATCON({path, id, ty=ity}, loc) =>
           let
             val ty = Ety.evalIty context ity
        ...
        let 
          val (varEnv1, tyFields, tppatFields) =
              foldr
                ...
                (VarMap.empty, SEnv.empty, SEnv.empty)
                fields
          val ty1 =
              if flex
              then
      ...
      | I.ICPATLAYERED {patVar as {path,id}, tyOpt, pat, loc} =>
        let
          ...
          val _ = 
              case tyOpt of
                NONE => ()
              | SOME ity => 
                let val ty2 = Ety.evalIty context ity
                        handle e => raise e
                in
                  U.unify [(ty1, ty2)]
                  handle U.Unify =>
  ...
  and typeinfFFIArg lambdaDepth applyDepth context ffiarg =
      case ffiarg of
        I.ICFFIARG (icexp, ffity, loc) =>
        let
          val (argTy, argExp) =
              TCU.freshInst (typeinfExp lambdaDepth applyDepth context icexp)
          val ffity = evalFFIty context (IMPORT {force=false}) ffity
          val stubTy = ffiStubTy ffity
          val _ =
      ...
      | I.ICFFIARGSIZEOF (ty, factorExpOpt, loc) =>
        let
          val ty = Ety.evalIty context ty
              handle e => raise e
          ...
          val argExp =
              ...
              | SOME ptfactorExp =>
                ...
                in
                  TC.TPPRIMAPPLY
                    {primOp = {primitive = BuiltinPrimitive.Word_mul,
                               ty = T.FUNMty ([BE.WORDty], BE.WORDty)},
                     instTyList = nil,
                     argExp = argPair,
                     loc = loc}
                end
  ...
  and monoTypeinfRule lambdaDepth argtyList context {args=patList,body=exp} = 
      ...
      in
        ...
        handle U.Unify =>
               let val ruleLoc = I.getRuleLocM [{args=patList, body=exp}]
               in
   ...
   *)
  and typeinfDeclList lambdaDepth isTop context nil = 
        (TIC.emptyContext, nil)
    | typeinfDeclList lambdaDepth isTop context (icdecl :: icdeclList) =  
      let 
        val (newContext1:TIC.context, tpdeclList1) =
            typeinfDecl lambdaDepth isTop context icdecl
(*
...
*)
   ...
   *)
  and typeinfDecl lambdaDepth isTop (context:TIC.context) icdecl =
    let
      val lambdaDepth = lambdaDepth
    in
      case icdecl of
        I.ICVAL (scopedTvars, icpatIcexpList, loc) =>
        let 
          val lambdaDepth = incDepth ()
          val (newContext, addedUtvars) =
                      ...
                      val (localBinds1, patternVarBinds1, extraBinds1) = 
                          ...
                          (
                           E.enqueueError "Typeinf 056-2"
                             (Loc.mergeLocs
                                (I.getLocPat icpat, I.getLocExp icexp),
                              exn);
                           (nil, nil, nil)
                          )
          ...
          fun bindVar (lambdaDepth, varEnv, var, varInfo as {ty, id, path}) =
              (TU.adjustDepthInTy lambdaDepth ty;
               VarMap.insert(varEnv, var, TIC.VARID varInfo))
          val newVarEnv = 
              foldl
              ...
              T.RECORDty
                (foldr
                   (fn ({funVarInfo={path, id, ty},...}, tyFields) =>
                       SEnv.insert(tyFields, String.concat path, ty))
                   SEnv.empty
                   funBindList)
                 ...
                 newContext) =>
                   TIC.bindVar
                     (
                      ...
                      newContext, 
                      {path=path, id=id},
                      if isTop then TIC.VARID funVarInfo
                      else TIC.RECFUNID (funVarInfo, length argTyList)
                     )
               )
               TIC.emptyContext
               funBindList,
      ...
      | I.ICNONRECFUN{guard, funVarInfo, rules, loc} =>
         let
           val lambdaDepth = lambdaDepth
           val icdecls = 
             case rules of
               {args = [pat], body} :: _ =>
               [(I.ICPATVAR(funVarInfo,loc),
                 I.ICFNM(rules, loc))]
             | [{args=patList as (pat::_), body}] =>
               let
                 val firstLoc = I.getLocPat pat
                 val lastLoc = I.getLocPat (List.last patList)
                ...
                (fn ({varInfo = var as {path,id}, body},
                    ...
                    in
                      (
                       ...
                       TIC.bindVar
                         (lambdaDepth, newContext, var, TIC.VARID varInfo)
                      )
                    end)
                (nil, newContext)
                recbinds
              ...
              T.RECORDty
                (foldr
                   (fn ({var={path,ty,id},...}, tyFields) =>
                       SEnv.insert(tyFields, String.concatWith "." path, ty))
                   SEnv.empty
                   varInfoTyTpexpList)
          val {boundEnv, ...} =
      ...
      | I.ICEXNTAGD ({exnInfo={path, id, ty=ity}, varInfo},loc) =>
        let
          val varInfo = 
              case VarMap.find(#varEnv context, varInfo)  of
                SOME (TIC.VARID varInfo) => varInfo
              | SOME (TIC.RECFUNID _) => 
                raise bug "recfunvar in ICEXNTAGD"
              | NONE => raise bug "var not found"
        in
          (TIC.emptyContext,
           [TC.TPEXNTAGD
              (
               {exnInfo = {path=path,id=id,ty=Ety.evalIty context ity},
                varInfo = varInfo},
               loc
              )
           ]
      ...
      | I.ICEXPORTFUNCTOR (var as {path, id}, ity, loc) =>
       (
         ...
         *)
        let
          val ty1 = Ety.evalIty context ity handle e => raise e
          val ty2 = case VarMap.find(#varEnv context, var) of
                     SOME (TIC.VARID {ty,...}) => ty
                   | SOME (TIC.RECFUNID({ty,...},_)) => ty
                   | NONE =>  raise bug "var not found"
                     ...
                     (let
                        ...
                        val tpexp =
                            TC.TPPOLY{btvEnv=boundtvars,
                                      ...
                                      exp=newTpexp,
                                      loc=loc}
                        val {id,ty,...} = TCU.newTCVarInfo ty1
                        val newDecl =
                            TC.TPVAL([({path=path,id=id,ty=ty}, tpexp)], loc)
                      in
                        (TIC.emptyContext,
                         [newDecl,TC.TPEXPORTVAR({path=path,id=id,ty=ty},loc)]
                        )
                      end
                     )
                   | _ => 
...
*)
      ...
      | I.ICEXPORTVAR (var as {path, id}, ity, loc) =>
        let
          val ty1 = Ety.evalIty context ity
              handle e => raise e
          val ty2 = case VarMap.find(#varEnv context, var) of
                     SOME (TIC.VARID {ty,...}) => ty
                   | SOME (TIC.RECFUNID({ty,...},_)) => ty
        ...
        in
          if TU.monoTy ty2 then
            (U.unify [(ty1, ty2)];
             (TIC.emptyContext,
              [TC.TPEXPORTVAR ({path=path, id=id, ty=ty1}, loc)]
             )
            )
            handle U.Unify =>
                   (E.enqueueError "Typeinf 074"
                      (loc, E.TypeAnnotationNotAgree
                              ("063-9",{ty=ty2,annotatedTy=ty1}));
                    (TIC.emptyContext,nil)
                   )
          else 
            let
              ...
              val (ty22, tpexp) =
                  TCU.freshInst (ty2, TC.TPVAR ({path=path,id=id,ty=ty2},loc))
            in
              (U.unify [(ty22, ty11)];
               let
                 val {boundEnv,...} = generalizer (ty11, lambdaDepth)
                 val (newTy, tpexp) = 
                     if BoundTypeVarID.Map.isEmpty boundEnv then
                       (ty11, tpexp)
                     else 
                       (T.POLYty {boundtvars = boundEnv, body = ty11},
                        TC.TPPOLY{btvEnv=boundEnv,
                                  expTyWithoutTAbs=ty11,
                                  exp=tpexp,
                                  loc=loc}
                       )
                 val {id,ty,...} = TCU.newTCVarInfo newTy
                 val newDecl =
                     TC.TPVAL([({path=path,id=id,ty=ty}, tpexp)], loc)
               in
                 (TIC.emptyContext,
              ...
              handle U.Unify =>
                     (E.enqueueError "Typeinf 075"
                        (loc, E.TypeAnnotationNotAgree
                                ("063-10",{ty=ty22,annotatedTy=ty11}));
                      (TIC.emptyContext,nil)
                     )
            end
        end
      | I.ICEXPORTEXN ({path, id, ty=ity}, loc) =>
        let
          val ty = Ety.evalIty context ity
              handle e => raise e
        in
      ...
      | I.ICOVERLOADDEF {boundtvars,id,path,overloadCase,loc} =>
        let
          ...
          fun substFTvar (subst as (ftvid, ty')) ty =
              ...
              | T.RECORDty tySenvMap =>
                T.RECORDty (SEnv.map (substFTvar subst) tySenvMap)
              | T.CONSTRUCTty {tyCon,args} =>
          ...
          fun typeinfOverloadMatch (tvId, expTy) {instTy, instance} =
              let
                ...
                val (actualTy, keyList, branch) =
                    case instance of
                      I.INST_OVERLOAD c => typeinfOverloadCase c
                    | I.INST_EXVAR ({path, ty}, loc) =>
                      let
                        val ty = Ety.evalIty context ty
                            handle e => raise e
            ...
            | typeinfOverloadMatches ty map (match::matches) =
              let
                ...
                val map =
                    ...
                    then raise bug "FIXME: user error: doubled tycon"
          ...
          and typeinfOverloadCase ({tvar,expTy,matches,loc}:I.overloadCase) =
              let
                val (tvStateRef, tvId) =
                ...
                val expTy = Ety.evalIty context expTy
                    handle e => raise e
                val matches =
                    map (fn {instTy, instance} =>
                            {instTy = Ety.evalIty context instTy
                                      handle e => raise e,
                             instance = instance})
                        matches
  ...
  fun typeinf icdecls =
      let
        ...
        val ({varEnv,...}, tpdecls) =
            typeinfDeclList T.toplevelDepth true TIC.emptyContext icdecls
        val tpdecls = 
            if E.isError() then 
              tpdecls
            else
              let
                ...
                val dummyTyPaths =
                    VarMap.foldli
                      (fn ({id, path}, TIC.VARID {ty,...}, paths) =>
                          if isDummy ty then path::paths
                          else paths
                        | ({id, path},TIC.RECFUNID ({ty,...},_),paths) => 
                          if isDummy ty then path::paths
                          else paths
                      )
                      nil
