...
struct
  ...
  fun mapToSEnv f nil = SEnv.empty
    | mapToSEnv f (h::t) =
      let
        val (label, value) = f h
      in
        SEnv.insert (mapToSEnv f t, label, value)
      end
  ...
  fun RECORD fields =
      let
        val recordTy = T.RECORDty (SEnv.map (fn (exp, expTy) => expTy) fields)
      in
        (fn loc => RC.RCRECORD
                     {fields = SEnv.map (fn (exp, expTy) => exp loc) fields,
                      recordTy = recordTy,
                      loc = loc},
         recordTy)
      end
  ...
  fun compileTy ty =
      ...
      | T.RECORDty fields =>
        T.RECORDty (SEnv.map compileTy fields)
  ...
  and compileExp context rcexp =
      ...
      | RC.RCRECORD {fields, recordTy, loc} =>
        RC.RCRECORD
          {fields = SEnv.map (compileExp context) fields,
                  ...
                  POLYFNM
                    (btvEnv, extraArgs,
                     LET (VALRECDEC (map (fn {localVar, exp, expTy, ...} =>
                                         ...
                                         recBinds),
                          RECORD (mapToSEnv (fn {localVar, label, ...} =>
                                                (label, Var localVar))
                                            recBinds)))
