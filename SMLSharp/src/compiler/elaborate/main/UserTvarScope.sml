...
struct
  ...
  type tvset = (A.eq * Loc.loc) SEnv.map
  type btvEnv = (A.eq * A.tvarKind) SEnv.map

  val empty = SEnv.empty : tvset
  val emptyEnv = SEnv.empty : btvEnv

  fun member (set, {name, eq}:A.tvar) =
      SEnv.inDomain (set, name)

  fun singleton ({name, eq}:A.tvar, loc) =
      SEnv.singleton (name, (eq, loc)) : tvset
  ...
  fun union (tvs1, tvs2) =
      SEnv.unionWithi
        (fn (name, tv1 as (eq1, loc1), (eq2, loc2)) =>
            (checkEqkind ({name=name, eq=eq2}, eq1, loc2); tv1))
        (tvs1, tvs2)
        : tvset

  fun toTvarList (tvset:tvset) =
      map (fn (n,(e,_)) => ({name=n, eq=e}, A.UNIV)) (SEnv.listItemsi tvset)
  ...
  fun extend (btvEnv:btvEnv, tvset:tvset) =
      SEnv.unionWith #2 (btvEnv, SEnv.map (fn (e,_) => (e, A.UNIV)) tvset)

  fun tyvarsOpt f (SOME x) = f x
    | tyvarsOpt f NONE = empty

  fun tyvarsList f l =
      foldl (fn (x, z) => union (f x, z)) empty l
  ...
  fun tyvarsTy btvEnv ty =
      ...
      | A.TYRECORD (rows, loc) =>
        tyvarsList (fn (k,t) => tyvarsTy btvEnv t) rows
  ...
  and tyvarsTvarKind btvEnv kind =
      case kind of
        A.UNIV => empty
      | A.REC (rows, loc) => tyvarsList (fn (k,t) => tyvarsTy btvEnv t) rows
  ...
  and tyvarsFFIty btvEnv ty =
      ...
      | P.FFIRECORDTY (rows, loc) =>
        tyvarsList (fn (k,t) => tyvarsFFIty btvEnv t) rows
...
*)
  ...
  fun tyvarsValdesc btvEnv (vid:string, ty) =
      tyvarsTy btvEnv ty

  fun tyvarsValdescList btvEnv valdescs =
      tyvarsList (tyvarsValdesc btvEnv) valdescs
...
*)
  ...
  and decideSpec spec =
      case spec of
        P.PLSPECVAL (desc as (_, valdescs, loc)) =>
        let
          val (_, scoped) = decideScope tyvarsValdescList emptyEnv desc
        in
          P.PLSPECVAL (scoped, valdescs, loc)
        end
      | P.PLSPECTYPE _ => spec
