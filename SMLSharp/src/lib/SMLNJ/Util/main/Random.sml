...
structure Random : RANDOM =
  struct
    ...
    val maxWord : Word32.word = 0wx7FFFFFFF             (* largest word *)
    val bit30 : Word32.word   = 0wx40000000
    val lo30 : Word32.word    = 0wx3FFFFFFF
    ...
    fun toString (RND{vals, borrow, congx, index}) = let
          ...
          fun fill (src,dst) =
                if src = N then ()
                  ...
                  P.update (arr, dst, Word32.toLargeWord (A.sub (vals, src)));
                  fill (src+1,dst+1)
                )
          in
            P.update (arr, 0, word0);
            P.update (arr, 1, LW.fromInt (!index));
            P.update (arr, 2, Word32.toLargeWord (!congx));
            fill (0,3);
    ...
    fun fromString s = let
          ...
          val congx = ref (Word32.fromLargeWord (subVec 2))
          val arr = A.array (N, 0w0 : Word32.word)
          fun fill (src,dst) =
                if dst = N then ()
                else (
                  A.update (arr, dst, Word32.fromLargeWord (subVec src));
                  fill (src+1,dst+1)
                )
          in
            fill (3, 0);
    ...
    val m : Word32.word = 0w2147483647
    val q = m div a
    val r = m mod a
    fun lcg seed = let
    ...
    fun randNat state = Word32.toIntX(randWord state & lo30)
    fun randReal state =
