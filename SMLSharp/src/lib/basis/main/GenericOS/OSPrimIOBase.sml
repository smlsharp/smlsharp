...
struct
  ...
  val getSTDIN = SMLSharp.Runtime.GenericOS_getSTDIN
  val getSTDOUT = SMLSharp.Runtime.GenericOS_getSTDOUT
  val getSTDERR = SMLSharp.Runtime.GenericOS_getSTDERR
  val fileClose : file_desc -> unit = SMLSharp.Runtime.GenericOS_fileClose
  val read' : file_desc * int -> Word8Vector.vector =
      fn (desc, bytes) =>
         _cast(SMLSharp.Runtime.GenericOS_fileRead (desc, bytes))
         : Word8Vector.vector
  val readbuf' : file_desc * Word8Array.array * int * int -> int =
      fn (desc, array, start, size) =>
         SMLSharp.Runtime.GenericOS_fileReadBuf
             (desc, _cast (array) : string, start, size)
  val writeVector : (file_desc * Word8Vector.vector * int * int) -> int =
      fn (desc, array, start, size) =>
         SMLSharp.Runtime.GenericOS_fileWrite
             (desc, _cast (array) : string, start, size)
  val writeArray : (file_desc * Word8Array.array * int * int) -> int =
      fn (desc, array, start, size) =>
         SMLSharp.Runtime.GenericOS_fileWrite
             (desc, _cast (array) : string, start, size)
  val fsetPos : file_desc * int -> int =
      SMLSharp.Runtime.GenericOS_fileSetPosition
  val fgetPos : file_desc -> int = SMLSharp.Runtime.GenericOS_fileGetPosition
  val fileno : file_desc -> int = SMLSharp.Runtime.GenericOS_fileNo
  val fileSize : file_desc -> int = SMLSharp.Runtime.GenericOS_fileSize
  val fileOpen : string * string -> file_desc =
      SMLSharp.Runtime.GenericOS_fileOpen
