...
structure CleanIO :> sig
    ...
    fun cleanUp C.AtExportML = doClean #flush
      | cleanUp C.AtExportFn = doClean #close
      | cleanUp C.AtExit = doClean #close
      | cleanUp C.AtInit = ((!osInitHook)(); (!stdStrmHook)(); doClean #init)
      | cleanUp C.AtInitFn = ((!osInitHook)(); (!stdStrmHook)(); doClean #init)
