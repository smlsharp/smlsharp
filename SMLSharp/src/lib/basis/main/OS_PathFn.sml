 ...
 * @version $Id: OS_PathFn.sml,v 1.4 2007/09/28 11:33:00 kiyoshiy Exp $
 *)
(* os-path-fn.sml
 *
...
functor OS_PathFn
            (OSPathBase
             : sig
                 ...
                 (** Split a string into the volume part and arcs part and
                  * note whether it is absolute.
  ...
  fun mkAbsolute {path, relativeTo} =
      case (fromString path, fromString relativeTo)
       of (_, {isAbs=false, ...}) => raise Path
        | ({isAbs=true, ...}, _) => path
        | ({vol=v1, arcs=al1, ...}, {vol=v2, arcs=al2, ...}) =>
          let
            fun mkCanon vol =
                mkCanonical
                    (toString{isAbs=true, vol=vol, arcs=List.@(al2, al1)})
          in
            if P.sameVol (v1, v2)
            then mkCanon v1
            else
              if (v1 = "")
              then mkCanon v2
              else if (v2 = "") then mkCanon v1 else raise Path
          end
          ...
          let
            ...
            fun strip (l, []) = mkArcs l
              ...
              | strip (l1 as (x1::r1), l2 as (x2::r2)) =
                if (x1 = x2) then strip (r1, r2) else dotDot (l1, l2)
            and dotDot (al, []) = al
            ...
            then raise Path
              ...
              case (al1, al2)
               of ([""], [""]) => currentArc
                ...
                | _ => toString{isAbs=false, vol="", arcs=strip(al1, al2)}
          end
      else raise Path

  fun isRoot path =
      case (fromString path)
       of {isAbs=true, arcs=[""], ...} => true
        | _ => false

  fun concat (p1, p2) =
      case (fromString p1, fromString p2)
          ...
          fun tr arc =
              ...
              then "."
                ...
                then ".."
                  ...
                  if Char.contains arc #"/" then raise Path else arc
        in
