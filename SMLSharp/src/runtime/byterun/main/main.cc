 ...
 * @version $Id: main.cc,v 1.32 2008/01/11 14:07:41 kiyoshiy Exp $
 */
/**
 * @author YAMATODANI Kiyoshi
 * @version $Id: main.cc,v 1.32 2008/01/11 14:07:41 kiyoshiy Exp $
...
usage(const char* exeName, const char* desc = ""){
    fprintf(stderr,
            "usage : %s [-heap heapsize] [-stack stack] -file filename\n"
            "        %s [-heap heapsize] [-stack stack] -client port\n"
            "        %s [-heap heapsize] [-stack stack] -server port\n"
            "        %s [-heap heapsize] [-stack stack] -pipe infd outfd\n"
            "        %s [-heap heapsize] [-stack stack] filename\n"
            "        %s {-help|-version}\n"
            "%s\n",
            exeName, exeName, exeName, exeName, exeName, exeName, desc);
    exit(1);
}
...
main(int argc, const char** argv)
{
    ...
    char* heapSizeStr = getEnvironment(IML_ENV_HEAPSIZE);
    if(heapSizeStr){heapSize = ::atoi(heapSizeStr);}
    if(0 == heapSize){heapSize = DEFAULT_HEAPSIZE;}
    char* stackSizeStr = getEnvironment(IML_ENV_STACKSIZE);
    if(stackSizeStr){stackSize = ::atoi(stackSizeStr);}
...
//    ChannelType channelType = CHANNEL_TYPE_UNSPECIFIED;
    ...
    while(nextArg < lastArg){
        ...
        else if(0 == strcmp("-help", nextArg[0])){
            usage(argv[0]);
        }
        else if(0 == strncmp("-", nextArg[0], 1)){
            usage(argv[0]);
        }
        else{
            break;
