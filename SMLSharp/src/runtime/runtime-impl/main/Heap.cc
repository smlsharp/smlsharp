 ...
 * @version $Id: Heap.cc,v 1.19 2008/01/18 05:42:46 kiyoshiy Exp $
 */
#include "Heap.hh"
...
const UInt32Value Heap::BLOCKTYPE_RECORD;
const UInt32Value Heap::BLOCKTYPE_ATOM;
const UInt32Value Heap::BLOCKTYPE_POINTER;
const UInt32Value Heap::BLOCKTYPE_SINGLE_ATOM_MUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_DOUBLE_ATOM_MUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_POINTER_MUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_SINGLE_ATOM_IMMUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_DOUBLE_ATOM_IMMUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_POINTER_IMMUTABLE_ARRAY;
const UInt32Value Heap::BLOCKTYPE_LARGEINT;

const UInt32Value Heap::SIZE_MASK;
const UInt32Value Heap::SIZE_SHIFT;
const UInt32Value Heap::TYPE_MASK;
const UInt32Value Heap::TYPE_SHIFT;
const UInt32Value Heap::FORWARDED_MASK;
const UInt32Value Heap::FORWARDED_SHIFT;
const UInt32Value Heap::MAX_RECORDBLOCK_PAYLOAD_SIZE;
const UInt32Value Heap::MAX_HEAPBLOCK_SIZE;
const UInt32Value Heap::WORDS_OF_HEADER;
const UInt32Value Heap::WORDS_OF_BITMAP;
const UInt32Value Heap::WORDS_OF_CELL;
const int Heap::BLOCK_ALIGNMENT;
const UInt32Value Heap::CELLS_OF_LARGEINT;
const int Heap::SIZE_OF_FINALIZABLE;
...
Heap::addFinalizable(Cell* block)
{
    ASSERT(BLOCKTYPE_RECORD == getType(BLOCK_TO_HEADER(block))
           || BLOCKTYPE_LARGEINT == getType(BLOCK_TO_HEADER(block)));
/*
...
Heap::isPointerField(BlockHeader* header, int index)
{
    ...
    switch(type)
    {
      ...
      case BLOCKTYPE_POINTER_MUTABLE_ARRAY:
      case BLOCKTYPE_POINTER_IMMUTABLE_ARRAY:
        return BOOLVALUE_TRUE;

      case BLOCKTYPE_ATOM:
      case BLOCKTYPE_SINGLE_ATOM_MUTABLE_ARRAY:
      case BLOCKTYPE_DOUBLE_ATOM_MUTABLE_ARRAY:
      case BLOCKTYPE_SINGLE_ATOM_IMMUTABLE_ARRAY:
      case BLOCKTYPE_DOUBLE_ATOM_IMMUTABLE_ARRAY:
      case BLOCKTYPE_LARGEINT:
...
Heap::invokeMajorGC()
{
    ...
    // reset 'visited' status which has been set in minor GC.
    clearVisitedOfFLOBs();
...
/**
 * In minor GC, forwarded bit of visited FLOB has been changed to 'true'.
 * If minor GC finds no space in elder-from region, before starting major GC,
 * we have to clear forwarded bit of FLOBs visited in minor GC, in order to
 * indicate that major GC has to scan and update the contents of FLOBs again.
 */
void
Heap::clearVisitedOfFLOBs()
{
    for (FLOBInfoMap::iterator i = FLOBInfoMap_.begin();
         i != FLOBInfoMap_.end();
         i++)
...
void
Heap::releaseUnreachableFLOBs()
{
    DBGWRAP(LOG.debug("start releaseUnreachableFLOBs");)
    DBGWRAP(int releasedFLOBs = 0;)
    FLOBInfoMap::iterator i = FLOBInfoMap_.begin();
    while(i != FLOBInfoMap_.end())
    {
        if(i->second->isReleased_ && !isForwarded(BLOCK_TO_HEADER(i->first)))
        {
//                DBGWRAP(LOG.debug("FLOB is freed: %x", i->first));
            ...
            delete i->second;
            // NOTE: 'map::erase' does not return new iterator.
            FLOBInfoMap_.erase(i++);
            DBGWRAP(releasedFLOBs += 1;)
        }
        else{
//                DBGWRAP(LOG.debug("FLOB is not canfree: %x", i->first));
    ...
    DBGWRAP(LOG.debug("finish releaseUnreachableFLOBs: %d", releasedFLOBs);)
}
...
/*
    DBGWRAP
    (LOG.debug
     ("checkReachabilityOfFinalizables:reachables = %d, unreachables = %d",
      reachableFinalizables_.size(),
      unreachableFinalizables_.size()));
*/
    ...
    /* 1st phase.
     * copy all unreachables and their descendants into the new heap area.
     */
...
Heap::runFinalizer()
{
     ...
     */
    if(inFinalizer){
        DBGWRAP(LOG.debug("Heap::runFinalizer finishes(false).");)
        return false;
    }
    ...
    try{
        ...
        {
            if(BLOCKTYPE_LARGEINT == getBlockType(*i)){
                LargeInt::release(*(largeInt*)*i);
            }
            else{
                try{
        ...
        inFinalizer = false;
        DBGWRAP(LOG.debug("Heap::runFinalizer finishes.");)
        return anyFinalized;
    }
    catch(...){
        inFinalizer = false;
...
Heap::isSimilarBlockGraph(Cell* block1, Cell* block2)
{
    ...
    switch(type1)
    {
      ...
      case BLOCKTYPE_DOUBLE_ATOM_MUTABLE_ARRAY:
      case BLOCKTYPE_POINTER_MUTABLE_ARRAY:
        /* if block1 == block2, this function has returned at the first line
         * above. */
        return false;

      case BLOCKTYPE_LARGEINT:
        return 0 == LargeInt::compare(*(largeInt*)block1, *(largeInt*)block2);
      ...
      case BLOCKTYPE_DOUBLE_ATOM_IMMUTABLE_ARRAY:
        for(int index = 0; index < size1; index += 1){
            ...
            if(field1.uint32 != field2.uint32){return false;}
        }
        return true;
      ...
      case BLOCKTYPE_POINTER_IMMUTABLE_ARRAY:
        for(int index = 0; index < size1; index += 1){
            Cell field1 = block1[index];
            Cell field2 = block2[index];
            if(false == isSimilarBlockGraph(field1.blockRef, field2.blockRef))
            {
                return false;
            }
        }
