...
static pthread_cond_t stop_the_world_cond;
...
#endif /* DEBUG */
{
	...
	do {
		pthread_cond_wait(&stop_the_world_cond, &giant_lock_mutex);
...
#define GIANT_UNLOCK_LIGHT() do { \
	int err ATTR_UNUSED; \
	err = pthread_cond_broadcast(&stop_the_world_cond); \
	ASSERT(err == 0); \
	err = pthread_mutex_unlock(&giant_lock_mutex); \
	ASSERT(err == 0); \
} while (0)
...
sml_control_init()
{
	...
	if (pthread_mutex_init(&giant_lock_mutex, NULL) != 0)
		sml_sysfatal("pthread_mutex_init failed");
#endif /* DEBUG */
	if (pthread_cond_init(&stop_the_world_cond, NULL) != 0)
...
sml_control_free()
{
	...
	pthread_cond_destroy(&stop_the_world_cond);
