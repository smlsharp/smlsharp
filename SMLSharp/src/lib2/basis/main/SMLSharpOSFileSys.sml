...
struct
  ...
  val prim_stat =
      ...
      : __attribute__((no_callback))
        (string, word SMLSharp.PrimArray.array) -> int
  val ya_GenericOS_fstat =
      _import "prim_GenericOS_fstat"
      : __attribute__((no_callback))
        (int, word SMLSharp.PrimArray.array) -> int
  ...
  struct
    fun isDir ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFDIR) = 0w0 then false else true
    fun isLink ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFLNK) = 0w0 then false else true
    fun isReg ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFREG) = 0w0 then false else true
    fun isChr ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFCHR) = 0w0 then false else true
    fun isBlk ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFBLK) = 0w0 then false else true
    fun isFIFO ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFIFO) = 0w0 then false else true
    fun isSock ({mode,...}:stat) =
        if SMLSharp.Word.andb (mode, S_IFSOCK) = 0w0 then false else true
    fun size ({size,...}:stat) =
