 ...
 * @version $Id: PrimitiveTable.sml,v 1.17 2008/05/31 12:18:23 ohori Exp $
 *)
...
struct
  ...
  fun hasEffectToBool "effective" = true
    | hasEffectToBool "noneffective" = false
    | hasEffectToBool string =
      raise Fail ("invalid hasEffect field :" ^ string)
      ...
      let
        ...
        fun parse (line, {lineno, index, dst}) =
            case line : CSVParser.field list of
              [SOME func, aliasOpt, SOME ty, SOME arity, SOME prim, NONE, SOME hasEffect] =>
              {
                lineno = lineno + 1,
                index = index,
                dst =
                {
                  ...
                  hasEffect = hasEffectToBool(#1 hasEffect)
                } :: dst
              }
            | [SOME func, aliasOpt, SOME ty, SOME arity, NONE, SOME vmFunc, SOME hasEffect] =>
              {
                lineno = lineno + 1,
                index = index + 1,
                dst =
            ...
            | [SOME func, aliasOpt, SOME ty, SOME arity, NONE, NONE, SOME hasEffect] =>
              {
                lineno = lineno + 1,
                index = index,
                dst =
          ...
          | format (dst, ({alias, typeSpec, ...} : spec) :: t) =
            let
              val code1 =
                  "val " ^ alias ^ "Ty = " ^
                  "TypeParser.readTy"^
(* ohori: Before counter refactoring
                  " (ReservedCounters.reservedBoundTypeVarIDSequence)" ^
*)
                  " PredefinedTypes.initialTopTyConEnv " ^
                  "\"" ^ typeSpec ^ "\" "
            in
                format (join "\n" (dst, code1), t)
            end
  ...
  fun primitiveInfosSML spec =
      let
        fun format (dst, nil) = dst
          | format (dst, ({bindName, alias, hasEffect, ...} : spec) :: t) =
            let
              val code =
                  "val " ^ alias ^ "PrimInfo = "
                  ^ "{name = \"" ^ bindName ^ "\", "
  ...
  fun primitivesSML spec =
      let
        fun format (dst, nil) = dst
          | format (dst, {bindName, alias, typeSpec, arity, primitive, hasEffect}::t) =
            let
              val instruction =
              ...
              val code =
                  ...
                  ^ "\"" ^ bindName ^ "\", "
                  ^"{bindName = \"" ^ bindName ^ "\", "
                  ^ "ty = \"" ^ typeSpec ^ "\", " (* ^ alias ^ "Ty, " *)
                  ^ "instruction = " ^ instruction ^ ", "
                  ^ "hasEffect = " ^ Bool.toString hasEffect ^ "}"
                  ^ ")"
            in
              format (join ",\n" (dst, code), t)
            end
