...
$Id: README.txt,v 1.10 2008/11/19 19:48:02 ohori Exp $
$Author: ohori $

This document describes the detail of how to add primitive operators,
...
By running the make command with the Makefile, source files are
generated from a primitives declaration file (primitives.csv) and
templates files (*.in).
               ...
               primitives.csv
                       ...
                       |
                       V
  Primitives.ppg.in ------> Primitives.ppg     
  Primitives.cc.in  ------> Primitives.cc

Similarly, from constant declaration file (constants.csv) and
templates files (*.in).
...
Informations about primitive operators are described in primitives.csv
in the CSV format. Each line in primitives.csv contains 6 fields. They are:
  ...
  7, effectiveness of primitive. A primitive is effective if it has
    side-effect, including I/O, raising exception, heap-block allocation.
...
It is error if both are specified.
 ...
 Example:
  ...
  +,add,"int * int -> int",2,ADDINT,noneffective
  :=,update,"['a.('a) ref * 'a -> unit]",2,,effective
  ----------------------------------------
...
'print' and ':=' have side-effect, while '+' does not.

====================
Template files

The *.in files in the compiler and runtime directory are templates,
from which source files to be compiled are generated. 
...
'@SML...@' and '@C...@' embedded in the texts are anchors which is
replaced with SML or C code snip generated from the primitives.csv. 
...
Names of constant integer value which are shared between the compiler
and the rumtime are declared in constants.csv file.
...
Similar with primitive declaration generation, anchor texts in *.in
files are replaced with constant declarations generated from
constants.csv.
...
For example, assume we add a new datatype dt that would be declared in
SML syntax as follows.
...
The second argument [false] indicates that dt takes a type parameter
which does not require equality.

-----
Then, register value constructors.

A) If these value constructors are not used in runtime source code,
addition of them to PredefinedTypes as follows is sufficient.
...
B) If the runtime source code uses these value constructors, you have
to add constructor tags to constants.csv. 
