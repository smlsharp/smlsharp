...
bool ensureRefOfBlock(Cell* ref)
{
    if(!Heap::isPointerField(ref, 0))
    {
        ...
        PrimitiveSupport::raiseFail("expects pointer value.");
        return false;
    }
    return true;
}
...
bool ensureFLOB(Cell* block)
{
    if(!Heap::isFLOB(block)){
        // ToDo : raise more specific exception.
        PrimitiveSupport::raiseFail("expects FLOB.");
        return false;
    }
    return true;
}
                         ...
                         Cell* resultRef)
{
    ...
    Cell* newBlock = Heap::fixedCopy(refBlock[0].blockRef);
    Cell newValue;
    newValue.blockRef = newBlock;
    Heap::updateField(argumentRefs[0]->blockRef, 0, newValue);
    *resultRef = PrimitiveSupport::constructUnit();
}
