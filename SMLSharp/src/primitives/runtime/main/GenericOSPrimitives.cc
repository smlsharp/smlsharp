...
raiseSysErr()
{
    ...
    DBGWRAP(LOG.error("raiseSysErr: %s \n", message);)
    errno = 0;
...
// ((int * word) array * (int * int) option)  -> (int * word) array
void
                           ...
                           Cell* resultRef)
{
    ...
    Cell* pollDescArray = argumentRefs[0]->blockRef;
    int pollDescArrayLength = PrimitiveSupport::getArrayLength(pollDescArray);
    struct pollfd fds[pollDescArrayLength];
    for(int index = 0; index < pollDescArrayLength; index += 1)
    {
        Cell pollDesc = pollDescArray[index];
        Cell pollDescElements[2];
        PrimitiveSupport::cellToTupleElements(pollDesc, pollDescElements, 2);
    ...
    int numberOfChanged =
        ::poll(fds, pollDescArrayLength, timeOutMilliSeconds);
    if(numberOfChanged < 0){
        raiseSysErr();
        return;
    }

    //  constructs an array including only the elements whose conditions are
    // enabled.
    int enables = 0;
    for(int index = 0; index < pollDescArrayLength; index += 1){
        short revents = fds[index].revents;
        if(revents){enables += 1;}
    }
    Cell resultArray;
    TemporaryRoot root1(&resultArray);
    resultArray.blockRef =
      PrimitiveSupport::allocatePointerArray(enables, true);
    ...
    for(int index = 0; index < pollDescArrayLength; index += 1){
        ...
        if(revents){
            ...
            Cell tuple = PrimitiveSupport::tupleElementsToCell(0, elements, 2);
            ASSERT(resultIndex < enables);
            Heap::initializeField(resultArray.blockRef, resultIndex, tuple);
            resultIndex += 1;
        }
    }
    *resultRef = resultArray;
    
    return;
#else /* HAVE_POLL_H */
    PrimitiveSupport::raiseFail("poll is not implemented.");
    return;
#endif 
}
...
#else /* HAVE_POLL_H */
    resultRef->uint32 = -1;
//    PrimitiveSupport::raiseFail("getPOLLINFlag is not implemented.");
    return;
#endif 
}
...
#else /* HAVE_POLL_H */
    resultRef->uint32 = -1;
//    PrimitiveSupport::raiseFail("getPOLLOUTFlag is not implemented.");
    return;
#endif 
}
...
#else /* HAVE_POLL_H */
    resultRef->uint32 = -1;
//    PrimitiveSupport::raiseFail("getPOLLPRIFlag is not implemented.");
    return;
#endif 
}
