...
fun rev l =
  let fun loop (l,nil) = l
        | loop (l,h::t) = loop (h::l,t)
  in loop (nil,l)
  end
val print = _import "prim_print" : __attribute__((no_callback)) string -> unit
val ya_String_allocateImmutableNoInit = _import "prim_String_allocateImmutableNoInit" : __attribute__((pure,no_callback,alloc)) word -> string

structure Int =
struct
  val toString = _import "prim_Int_toString" : __attribute__((pure,no_callback,alloc)) int -> string
end

fun x ^ y =
    let
      val n1 = PrimString.size x
      val n2 = PrimString.size y
      val newstr = ya_String_allocateImmutableNoInit (Word.fromInt (n1 + n2))
    in
      PrimString.copy_unsafe (x, 0, newstr, 0, n1);
      PrimString.copy_unsafe (y, 0, newstr, n1, n2);
      newstr
    end
...
struct
  ...
  fun tabulate (n, f) =
      if n < 0 then raise Size
      else if n = 0 then Array.vector(0, f 0)
      else
        let val e = f 0
            val v = Array.vector (n, e)
            fun loop i =
                if i < n
                then (Array.update (v, i, f i); loop (i+1))
                else ()
        in loop 0; v
      end
end
...
struct
  ...
  fun tabulate (n, f) =
      let fun loop i = if i < n then f i :: loop (i+1) else nil
      in loop 0
      end
end
