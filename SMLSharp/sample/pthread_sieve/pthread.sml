...
struct
  ...
  val sizeof_pthread_mutex_t =
      CConfig.checkSizeOf("pthread_mutex_t", ["pthread.h"])
  val sizeof_pthread_cond_t =
      CConfig.checkSizeOf("pthread_cond_t", ["pthread.h"])
  val _ = CConfig.saveCache ()

  type pthread_mutex_t = Word8Array.array
  type pthread_cond_t = Word8Array.array

  fun new_pthread_t_ref () = ref NULL
