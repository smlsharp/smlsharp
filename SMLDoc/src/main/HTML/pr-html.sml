 ...
 * @version $Id: pr-html.sml,v 1.2 2004/10/20 03:33:58 kiyoshiy Exp $
 *)
...
*)
	  ...
	  in
          "<" ^ tag ^ (String.concat(map (fn attr => " " ^ attr) attrs)) ^ ">"
(*
	    ListFormat.fmt {
		init = "<",
		sep = " ",
...
*)
   ...
   **)
    ...
    and prTableRows (strm, rows) = let
	  fun prTR (HTML.TR{align, valign, bgcolor, content}) = (
		prTag (strm, "TR", [
		    ("ALIGN", hAlign align),
    ...
    and prText (strm, text) = (case text
	   of (HTML.TextList tl) =>
	    ...
	    | (HTML.A{name, href, rel, rev, title, target, content}) => (
		prTag (strm, "A", [
		    ("NAME", CDATA name),
		    ("HREF", CDATA href),
		    ("REL", CDATA rel),
    ...
    fun prFrame strm (HTML.Frame{src, name}) =
        (
          prTag(strm, "FRAME", [("SRC", CDATA src), ("NAME", CDATA name)])
        )
      | prFrame strm (HTML.FrameSet{cols, rows, frames}) =
        (
          prTag
            (strm, "FRAMESET", [("COLS", CDATA cols), ("ROWS", CDATA rows)]);
          List.app (prFrame strm) frames;
          prEndTag (strm, "FRAMESET")
        )
    ...
    fun prBody (strm, HTML.BODY{
	  ...
	  prBlock (strm, content);
	  prEndTag (strm, "BODY"))
      | prBody (strm, HTML.FRAMEBODY{
          frame, noframes
        }) = (
          prFrame strm frame;
          prTag (strm, "NOFRAMES", []);
          prBlock (strm, noframes);
          prEndTag (strm, "NOFRAMES"))
    ...
    fun prHTML {putc, puts} html = let
	  ...
	  in
	    case version
	     of NONE => ()
	      | (SOME v) =>
                puts ("<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML " ^ v ^ "//EN\">\n")
(*
                puts (F.format
