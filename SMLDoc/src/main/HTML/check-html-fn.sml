 ...
 * @version $Id: check-html-fn.sml,v 1.2 2004/10/20 03:33:57 kiyoshiy Exp $
 *)
functor CheckHTMLFn (Err : HTML_ERROR) : sig
    ...
    fun check context (HTML.HTML{body=HTML.BODY{content, ...}, ...}) =
	  ...
	  fun error (elem, ctx) =
	      Err.syntaxError
                  context ("unexpected " ^ elem ^ " element in " ^ ctx)
	  fun contentError ctx =
		Err.syntaxError context ("unexpected element in " ^ ctx)
	  fun formError elem =
		Err.syntaxError
                    context ("unexpected " ^ elem ^ " element not in FORM")
	  fun attrError attr = Err.missingAttr context attr
	  fun checkBodyContent {inForm} b =
              (case b of
	  ...
	  and checkBlock {inForm} blk =
		 ...
		 checkBodyContent {inForm=inForm} content
	       ...
	       | (HTML.ADDRESS _) => error ("ADDRESS", "block")
		                     (* end case *))
	  and checkItems {inForm, inDirOrMenu} items =
              let
	  ...
	  and checkText {inAnchor, inForm, inPre, inApplet} =
              let
		fun chk txt =
                    (case txt of
		       (HTML.TextList tl) => List.app chk tl
		     | (HTML.PCDATA _) => ()
		     | (HTML.TT txt) => chk txt
		     | (HTML.I txt) => chk txt
		     | (HTML.B txt) => chk txt
		     ...
		     | (HTML.BIG txt) =>
                       (if inPre then error("BIG", "PRE") else (); chk txt)
		     | (HTML.SMALL txt) =>
                       (if inPre then error("SMALL", "PRE") else (); chk txt)
		     | (HTML.SUB txt) =>
                       (if inPre then error("SUB", "PRE") else (); chk txt)
		     | (HTML.SUP txt) =>
                       (if inPre then error("SUP", "PRE") else (); chk txt)
		     | (HTML.EM txt) => chk txt
