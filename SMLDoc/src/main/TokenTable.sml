...
(*
	  ("use"	, fn yypos => Tokens.USE(yypos,yypos+3)),
*)
          ("_atom"      , fn yypos => Tokens.ATOM(yypos,yypos+5)),
          ("_boxed"     , fn yypos => Tokens.BOXED(yypos,yypos+6)),
          ("_cast"      , fn yypos => Tokens.CAST(yypos,yypos+5)),
          ("_cdecl"     , fn yypos => Tokens.CDECL(yypos,yypos+6)),
          ("_double"    , fn yypos => Tokens.DOUBLE(yypos,yypos+7)),
          ("_export"    , fn yypos => Tokens.EXPORT(yypos,yypos+7)),
          ("_ffiapply"  , fn yypos => Tokens.FFIAPPLY(yypos,yypos+9)),
          ("_import"    , fn yypos => Tokens.IMPORT(yypos,yypos+7)),
          ("_sizeof"    , fn yypos => Tokens.SIZEOF(yypos,yypos+7)),
          ("_stdcall"   , fn yypos => Tokens.STDCALL(yypos,yypos+8)),
          ("_useobj"    , fn yypos => Tokens.USEOBJ(yypos,yypos+7))
        ]
