...
val image = Array.tabulate (size * size, fn _ => 0w0 : word8)
...
fun mandelbrot x y w h =
    ...
    then let val w2 = w div 2
             val t = Thread.create (fn _ => mandelbrot (x + w2) y (w - w2) h)
         in mandelbrot x y w2 h + Thread.join t
         end
    else let val h2 = h div 2
             val t = Thread.create (fn _ => mandelbrot x (y + h2) w (h - h2))
         in mandelbrot x y w h2 + Thread.join t
         end
