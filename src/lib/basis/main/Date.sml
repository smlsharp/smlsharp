...
struct
   ...
   * the local time zone
   *)
  val ascTime : tm -> string = Date_ascTime
  val localTime' : Int32.int -> tm = Date_localTime
  val gmTime' : Int32.int -> tm = Date_gmTime
  val mkTime' : tm -> Int32.int = Date_mkTime
  val strfTime : (string * tm) -> string = Date_strfTime
  ...
  local 
    ...
    fun sum (f, k, p) = 
        let
          fun loop (f, i, p, acc) =
              if (not(p(i))) then acc else loop(f, i + 1, p, acc + f(i))
        in
          loop (f, k, p, 0)
        end
    fun lastDayOfGregorianMonth (month, year) =
        if
          ((month = 1)
           andalso (Int.mod (year, 4) = 0)
           andalso not (Int.mod (year, 400) = 100)
           andalso not (Int.mod (year, 400) = 200)
           andalso not (Int.mod (year, 400) = 300))
        then 29
  ...
  fun toAbsolute (month, day, year) =
      day  
      + sum
        (fn (m) => lastDayOfGregorianMonth(m, year), 0, fn (m) => (m < month))
      + 365 * (year -1)
      + quot (year-1, 4)
      - quot (year-1, 100)
      + quot (year-1, 400)
  fun fromAbsolute (abs) =
      let
        val approx = quot (abs, 366)
        val year =
            approx
            + sum
              (fn (_) => 1, approx, fn (y) => (abs >= toAbsolute(0, 1, y + 1)))
        val month =
            sum
            (
              fn (_) =>1, 0,
              fn (m) =>
                 (abs > toAbsolute(m, lastDayOfGregorianMonth(m, year), year))
            )
  ...
  fun yday (month, day, year) = 
      let
        val abs = toAbsolute (month, day, year)
        val daysPrior = 
            365 * (year -1)
            + quot (year - 1, 4)
            - quot (year - 1, 100)
