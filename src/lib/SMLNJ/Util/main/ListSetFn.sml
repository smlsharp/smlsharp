...
functor ListSetFn (K : ORD_KEY) :> ORD_SET where type Key.ord_key = K.ord_key =
  ...
  (* sets are represented as ordered lists of key values *)
    ...
    fun add (l, item) =
        let
	  fun f lessers [] = List.revAppend (lessers, [item])
	    | f lessers (elem::r) =
              (case Key.compare(item, elem)
		of LESS => List.revAppend (lessers, item :: elem :: r)
		 | EQUAL => List.revAppend (lessers, item :: r)
		 | GREATER => f (elem :: lessers) r)
	in
	  f [] l
	end
(*
