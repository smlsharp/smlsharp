...
functor SplayMapFn (K : ORD_KEY) : ORD_MAP =
  ...
  (* Look for an item, return NONE if the item doesn't exist *)
    ...
    local
      fun next ((t as SplayObj{right, ...})::rest) = (t, getLeft(right, rest))
	| next _ = (SplayNil, [])
      and getLeft (SplayNil, rest) = rest
	| getLeft (t as SplayObj{left=l, ...}, rest) = getLeft(l, t::rest)
    in
