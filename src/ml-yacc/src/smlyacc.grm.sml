...
sharing type Parser.pos = ParserData.pos
sharing type Parser.arg = ParserData.arg
sharing type Parser.result = ParserData.result
...
structure Parser = LrParser()
structure Token = Parser.Token
...
\\036\000\050\000\040\000\045\000\
\\048\000\047\000\046\000\032\000\
...
fn (T 0) => "ARROW"
  ...
  | (T 12) => "IDDOT"
  | (T 13) => "PERCENT_FOOTER"
  | (T 14) => "PERCENT_DECOMPOSE"
  | (T 15) => "PERCENT_BLOCKSIZE"
  | (T 16) => "PERCENT_HEADER"
  | (T 17) => "INT"
  | (T 18) => "KEYWORD"
  | (T 19) => "LBRACE"
...
structure Token = ParserData.Parser.Token
structure Parser =
struct
type pos = ParserData.pos
type token = (ParserData.svalue, pos) Token.token
type arg = ParserData.arg
type result = ParserData.result
type stream = token ParserData.Parser.Stream.stream
exception ParseError = ParserData.Parser.ParseError
fun makeStream {lexer : unit -> token} =
    ParserData.Parser.Stream.streamify lexer
val consStream = ParserData.Parser.Stream.cons
val getStream = ParserData.Parser.Stream.get
val sameToken = ParserData.Token.sameToken
    val parse = fn {lookahead,stream=lexer,error,arg} =>
       (fn (a,b) => (ParserData.Actions.extract a,b))
     (ParserData.Parser.parse {table = ParserData.table,
               lexer=lexer,
               lookahead=lookahead,
...
type token = Parser.token and pos = Parser.pos
...
fun PERCENT_FOOTER (p1,p2) = Token.TOKEN (ParserData.LrTable.T 13,(
ParserData.MlyValue.VOID,p1,p2))
fun PERCENT_DECOMPOSE (p1,p2) = Token.TOKEN (ParserData.LrTable.T 14,(
ParserData.MlyValue.VOID,p1,p2))
fun PERCENT_BLOCKSIZE (p1,p2) = Token.TOKEN (ParserData.LrTable.T 15,(
ParserData.MlyValue.VOID,p1,p2))
fun PERCENT_HEADER (p1,p2) = Token.TOKEN (ParserData.LrTable.T 16,(
ParserData.MlyValue.VOID,p1,p2))
