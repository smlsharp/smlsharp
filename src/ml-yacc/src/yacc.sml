    ...
    val printTokenStruct =
	 ...
	 NAMES {miscStruct,tableStruct,valueStruct,
		...
		(sayln ("structure " ^ tokenStruct ^ " : " ^ tokenSig ^ " =");
		 ...
		 sayln "type token = Parser.token and pos = Parser.pos";
    ...
    val printSigs = fn (VALS {term,tokenInfo,...},
		...
		(case tokenInfo of NONE => "" | SOME s => (s^"\n"))^
		 ...
		 "end\nsignature " ^ miscSig ^
		  ...
		  "\nstructure Parser : SMLYACC_PARSER" ^
		  "\nsharing type Parser.pos = " ^ dataStruct ^ ".pos" ^
		  "\nsharing type Parser.arg = " ^ dataStruct ^ ".arg" ^
		  "\nsharing type Parser.result = " ^ dataStruct ^ ".result" ^
...
(* 2012-9-30 %decompose and %blockSize *)
fn (decompose_decl, blockSize_decl) =>
...
(* 2019-07-06: By Ohori, smlyacc have avoided functor usage. *)
	    of NONE => (say "structure "; say miscStruct;
			say " : sig structure ";
			say dataStruct;
			say " : "; sayln dataSig;
			say "       structure ";
			say tokenStruct; say " : "; sayln tokenSig;
			sayln "       structure Parser : SMLYACC_PARSER";
			sayln "   end")
	     | SOME s => say s;
	    sayln " = ";
	    sayln "struct";
	    ...
	    sayln header;
	    sayln "end";
	    sayln "structure Parser = LrParser()";
	    sayln "structure Token = Parser.Token";
	    ...
	    printAction(rules,values,names)(decompose_decl,blockSize_decl);
	    sayln "end";
	    say ("\
\structure Token = "^dataStruct^".Parser.Token\n\
\structure Parser =\n\
\struct\n\
\type pos = "^dataStruct^".pos\n\
\type token = ("^dataStruct^".svalue, pos) Token.token\n\
\type arg = "^dataStruct^".arg\n\
\type result = "^dataStruct^".result\n\
\type stream = token "^dataStruct^".Parser.Stream.stream\n\
\exception ParseError = "^dataStruct^".Parser.ParseError\n\
\fun makeStream {lexer : unit -> token} =\n\
\    "^dataStruct^".Parser.Stream.streamify lexer\n\
\val consStream = "^dataStruct^".Parser.Stream.cons\n\
\val getStream = "^dataStruct^".Parser.Stream.get\n\
\val sameToken = "^dataStruct^".Token.sameToken\n\
\    val parse = fn {lookahead,stream=lexer,error,arg} =>\n\
\       (fn (a,b) => ("^dataStruct^".Actions.extract a,b))\n\
\     ("^dataStruct^".Parser.parse {table = "^dataStruct^".table,\n\
\               lexer=lexer,\n\
\               lookahead=lookahead,\n\
\               saction = "^dataStruct^".Actions.actions,\n\
\               arg=arg,\n\
\               void= "^dataStruct^".Actions.void,\n\
\               ec = {is_keyword = "^dataStruct^".EC.is_keyword,\n\
\                     noShift = "^dataStruct^".EC.noShift,\n\
\                     preferred_change = "^dataStruct^".EC.preferred_change,\n\
\                     errtermvalue = "^dataStruct^".EC.errtermvalue,\n\
\                     error=error,\n\
\                     showTerminal = "^dataStruct^".EC.showTerminal,\n\
\                     terms = "^dataStruct^".EC.terms}}\n\
\      )\n\
\end\n\
\");
	    printTokenStruct(values,names);
	    sayln "end";
(* 2012-1-13 ohori %footer *)
