...
local
 ...
 (* This is to avoid name conflict in functor names and variable names *)
  ...
  type path = SymbolWithLoc.symbol list
             ...
             (case VP.findCon(env, refLongsymbol) of
                ...
                (EU.enqueueError
                   (SymbolWithLoc.longsymbolToLoc refLongsymbol,
                    E.ConNotFound("020", {longsymbol = SymbolWithLoc.toLongsymbol refLongsymbol}));
...
(* 2012-9-25 ohori added to fixe 241_functorExn bug *)
  ...
  fun evalPdecl (renameEnv:renameEnv) (path:SymbolWithLoc.symbol list)
      ...
      | P.PDREPLICATEDAT (symbol, longsymbol, loc) =>
        (case (VP.findTstr (env, longsymbol)) handle e => raise e of
           NONE => (EU.enqueueError
                      (SymbolWithLoc.longsymbolToLoc longsymbol,
                       E.DataTypeNameUndefined("140", {longsymbol = SymbolWithLoc.toLongsymbol longsymbol}));
                    (renameEnv, V.emptyEnv, nil))
         | SOME (sym, tstr) => 
           let
             val (tstr, renameEnv) = 
      ...
      | P.PDABSTYPE (datadeclList, withty, pdeclList, loc) =>
        let
          ...
          fun abstractTyE tyE tfvSubst =
              ...
              tfvSubst
              tyE
          val (env1 as V.ENV {varE, tyE, strE}, _) = Ty.evalDatatype env (datadeclList, withty, loc)
...
*)
  ...
  *)
                       ...
                       | SOME _ =>
                         (EU.enqueueError
                            (SymbolWithLoc.longsymbolToLoc longsymbol,
                             E.ExnExpected("170", {longsymbol = SymbolWithLoc.toLongsymbol longsymbol}));
                          (exEnv, exdeclList)
                         )
                       | NONE => 
                         (EU.enqueueError
                            (loc,E.ExnUndefined("160",{longsymbol = SymbolWithLoc.toLongsymbol longsymbol}));
                          (exEnv, exdeclList)
                         )
                      )
                )
          ...
          let
            val idstatus = 
                ...
                | NONE => 
                  (EU.enqueueError
                     (SymbolWithLoc.longsymbolToLoc refLongsymbol,
                      E.VarNotFound("190",{longsymbol=SymbolWithLoc.toLongsymbol refLongsymbol}));
      ...
      | P.PLSTRID longsymbol =>
        (case VP.findStr(env,longsymbol) of
         ...
         | NONE => 
           ...
           in
             (EU.enqueueError (loc, E.StrNotFound("430",{longsymbol = SymbolWithLoc.toLongsymbol longsymbol}));
        ...
        *)
        ...
        fun exnTagsVarE path varE exnTags =
            ...
            (fn (name, idstatus, exnTags) => 
                ...
                | I.IDSPECEXN _ => (SymbolWithLoc.prefixPath' (path, name))::exnTags
        ...
        fun genExportVarE exnInfoList exnPathMap path (version, RVarE) icdecls =
            SymbolWithLocEnv.foldli
              ...
              (fn (name, idstatus, (exnInfoList, exnPathMap, varE, icdecls)) =>
                  let
                    val exLongsymbol = SymbolWithLoc.prefixPath' (path, name)
  ...
  local
    fun setUsedflagInOverloadMatch (env:V.topEnv) match =
        case match of
          ...
          let
            val idstatus = 
                case VP.findId (#Env env, SymbolWithLoc.fromLongsymbol path) of
                  SOME (sym, idstatus) => idstatus
