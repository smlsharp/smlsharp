(**
 * hanlding command line options.
 * @copyright (c) 1998 Bell Labs, Lucent Technologies.
 * @version $Id: GetOpt.sml,v 1.2 2005/12/27 11:00:42 kiyoshiy Exp $
 *)
...
struct
  ...
  fun usageInfo {header, options} =
      let
        ...
        val (ms1, ms2) =
            foldl
                (fn ((e1, e2,_), (m1, m2)) =>
                    (Int.max (size e1, m1), Int.max (size e2, m2)))
                (0,0)
                fmtOptions
        ...
        val table =
            foldr
                (fn ((e1, e2, e3), l) =>
                    (concat ["  ", pad ms1 e1, "  ", pad ms2 e2, "  ", e3])
                    :: l)
                []
                fmtOptions
      in
   ...
   *)
  fun getOpt {argOrder, options : 'a opt_descr list, errFn} =
      let
        ...
        fun errReq (d, optStr) =
            errFn(concat["option `", optStr, "' requires an argument ", d])
        fun errUnrec optStr =
            errFn(concat["unrecognized option `", optStr, "'"])
        fun errNoArg optStr =
            errFn(concat["option `", optStr, "' does not allow an argument"])
        (* handle long option
         * this is messy because you cannot pattern-match on substrings
         *)
        fun longOpt (subs, rest, optDescr : 'a opt_descr list) =
            let
         ...
         * options.
         *)
        fun shortOpt (x, subs, rest, optDescr : 'a opt_descr list) =
            let 
              val options =
                  List.filter
         ...
         * "-" is interpreted as a general argument.
         *)
        fun getNext ([], _) = raise Fail "getNext: impossible"
          | getNext ("--" :: rest, _) = (EndOfOpts, rest)
          | getNext ((x as "-") :: rest, _) = (NonOpt x, rest)
        ...
        fun get [] = ([], [])
          | get args =
            let val (opt, rest) = getNext (args, options)
            in
              case opt of
                Opt o' => let val (os, xs) = get rest in (o' :: os, xs) end
              | NonOpt x =>
                let val (os, xs) = get rest
                in
                  case argOrder of
                    RequireOrder => ([],x :: rest)
                  | Permute => (os, x :: xs)
                  | ReturnInOrder f => ((f x) :: os, xs)
                end
              | EndOfOpts =>
                case argOrder of
                  RequireOrder => ([], rest)
                | Permute => ([], rest)
                | ReturnInOrder f => (map f rest, [])
            end
      in
        get
      end (* getOpt *)
