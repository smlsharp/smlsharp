...
fun varIdentInfoToVarPathInfo {ident=_, displayName=n, ty=ty} =
...
(* The alias needs not to be checked. *)
    ...
    | FRAMEBITMAPty is => ()
    | ABSSPECty (ty1,ty2) => (checkType env ty1 loc
                  (* ; checkTyOpt env ty2 loc *)
                  )
...
fun checkExp env exp =
    (
    case exp of
      TFPFOREIGNAPPLY (expFn, tylistInst, expArg, tylistArgs, loc) =>
    ...
    | TFPARRAY (expSize,expInitVal,tyElem,tyArr,loc) =>
    ...
    | TFPPRIMAPPLY (primInfo,tylist,expoption,loc) =>
    ...
    | TFPOPRIMAPPLY (oprimInfo, tyList,expOption,loc) =>
    ...
    | TFPCONSTRUCT (conIdInfo,tyList,expOption,loc) =>
    ...
    | TFPMONOLET (varIdInfoExpList,exp,loc) =>
    ...
    | TFPLET (declList,expList,tyList,loc) => 
    ...
    | TFPMODIFY (label,expRec,tyRec,exp,ty,loc) => 
      let val _ = checkType env tyRec loc
          ...
          val tySmap = 
              ...
              | _ => raise ERREXP loc
                     (TF.tyToString tyRec' ^ 
                      " is not a record type in TFPMODIFY")
    ...
    | TFPHANDLE (exp,varIdentInfo,expHandler,loc) => 
    ...
    | TFPCASE (exp,tye,patExpList,tyr,caseKind,loc) => 
      let val _ = checkTypes env [tye,tyr] loc
          val ty = checkExp env exp
          ...
          val (ty1,ty2) = checkPatExpList env patExpList loc
          ...
          val _ = if eqType env tyr ty2 = false
                  then raise ERREXP loc (
                           "type annotation mismatch in case body expression" ^
                           "\n  expression: " ^
                           TF.tyToString ty2 ^
                           "\n  annotation: " ^
                           TF.tyToString tyr)
    ...
    | TFPFN (varIdentInfo,ty,exp,loc) => 
      let val _ = checkType env ty loc
          val ty1 = #ty varIdentInfo
          val id = #displayName varIdentInfo
    ...
    | TFPPOLYFN (btvs,varIdentInfo,ty,exp,loc) => 
      let val env = extendBtvEnv env btvs
...
and checkValIdentExpList env valIdentExpList loc =
    let fun checkLoop env varEnv []                      = varEnv
          | checkLoop env varEnv ((valIdent,exp)::binds) =
            let val (vty,isWild,varIdentInfoOpt) = 
                    case valIdent of
                      VALDECIDENT varIdentInfo => (#ty varIdentInfo, false,
                                             SOME varIdentInfo)
                    | VALDECIDENTWILD ty    => (ty, true, NONE)
                val ty   = checkExp env exp
                val _ =
                ...
                val varEnv' = 
                    if isWild 
                    then SEnv.empty
                    else let val varIdentInfo 
                                 as {ident=_,displayName=n,ty=ty} =
                                 valOf varIdentInfoOpt
                         in  SEnv.singleton 
                               (n, 
                                VARID (varIdentInfoToVarPathInfo varIdentInfo))
                         end
...
and checkVarIdInfoExpList env varIdInfoExpList loc =
    let val valIdentExpList = 
            map (fn (varIdInfo,exp) => 
                       (VALDECIDENT varIdInfo,exp)) varIdInfoExpList
             ...
             foldl
              (fn ((varIdInfo,ty,exp),(varEnv,binds)) =>
               ...
               in (mergeVarEnv varEnv
                      (SEnv.singleton
                         (id,VARID (varIdentInfoToVarPathInfo varIdInfo))),
                   binds @ [(varIdInfo,exp)])
...
and checkPat env pat =
    ...
    | TFPPATVAR (varIdInfo,loc) => 
      let val id = #displayName varIdInfo
          val ty = #ty varIdInfo
          val _ = checkType env ty loc
      in  (ty, SEnv.singleton (id,
                   VARID (varIdentInfoToVarPathInfo varIdInfo)))
      end
    ...
    | TFPPATCONSTRUCT (conIdInfo, tyList, patOption, ty, loc) =>
      let val _ = checkTypes env tyList loc
          val _ = checkType env ty loc

          val annty = ty

          val id       = #displayName conIdInfo

          val polyty   = #ty conIdInfo
          val polyty   = skip polyty
          ...
          val ty = if eqType env annty ty = false
                   then raise ERRPAT loc (
                          "type annotation mismatch in constructor pattern" ^
                          "\n  constructor: " ^
                          #displayName conIdInfo ^
                          "\n  type: " ^
                          TF.tyToString ty ^
    ...
    | TFPPATRECORD (patfields, ty, loc) =>
      let val _ = checkType env ty loc
          ...
          case ty of
            RECORDty fieldEnv'
            => if leqRecKind env (REC fieldEnv) (REC fieldEnv') = false
               then raise ERRPAT loc
                      ("type annotation mismatch in record pattern" ^
                       "\n  pattern: " ^
                       TF.tyToString recty ^
                       "\n  annotation: " ^
                       TF.tyToString ty)
          ...
          | BOUNDVARty i 
            => (case IEnv.find (#btvEnv env,i) of
                  ...
                  => if leqRecKind env (REC fieldEnv) (REC fieldEnv') = false
                     then raise ERRPAT loc
                           ("record kind mismatch in record pattern" ^
                            "\n  pattern: " ^
                            TF.tyToString recty ^ 
                            "\n  annotation: " ^
                            TF.tyToString ty)
                ...
                | SOME {recKind=UNIV,...}
                  => raise ERRPAT loc
                      ("type annotation mismatch in record pattern (not record kind)" ^
                       TF.tyToString ty)
                | _ 
                  => raise ERRPAT loc
                      ("type annotation mismatch in record pattern (unbound type variable)" ^
                       TF.tyToString ty)
               )
          | _ => raise ERRPAT loc
                      ("type annotation mismatch in record pattern (not record type) " ^
                       TF.tyToString ty)
      in  (ty, patvarEnv)
      end

    | TFPPATLAYERED (pat, pat', loc) =>
      let val varIdInfo = 
                case pat of
                  TFPPATVAR (varIdInfo,loc) => varIdInfo
                | _ => raise ERRPAT loc ("ill-formed layered pattern (non-variable pattern)")
          val (ty,patvarEnv) = checkPat env pat'
          val vty = #ty varIdInfo

          val _ = if eqType env ty vty = false
                  then raise ERRPAT loc
                        ("type annotation mismatch in layered pattern" ^
                         "\n  pattern: " ^
                         TF.tyToString ty ^ 
                         "\n  annotation: " ^
                         TF.tyToString vty)
...
and checkPatExpList env [] loc = raise ERRPAT loc ("empty matches")
  | checkPatExpList env [patbind] loc = checkPatExp env patbind
  | checkPatExpList env (patbind::patbinds) loc =
    let val (ty1,ty2) = checkPatExp env patbind
        val (ty1',ty2') = checkPatExpList env patbinds loc
    in  if eqType env ty1 ty1' = false
        then raise ERRPAT loc (
               "pattern types of rules don't agree" ^
               "\n  this rule: " ^
               TF.tyToString ty1 ^ 
               "\n  the next rule(s): " ^
               TF.tyToString ty1')
...
and checkDecl env decl =
  ...
  | TFPSETFIELD (expVal, expArr, i, ty, loc) =>
    let val _ = checkType env ty loc
