...
Last, compose them by using 'data' to define a combinator for the datatype.
  ...
  local
    ...
    fun pu_E pu_dt = P.con1 (fn (E n) => n) (fn n => E n) P.int
    fun pu_F pu_dt = P.con1 (fn (F dt) => dt) (fn dt => F dt) pu_dt
  in
...
Combinators for them are defined as follows.
  ...
  val newPu_dt1 : pu_dt1 P.pu =
      let
        fun dt1ToInt D = 0
          | dt1ToInt (E _) = 1
        fun pu_D pu_dt1 = P.con0 D pu_dt1
        fun pu_E pu_dt1 = P.con1 (fn (E n) => n) (fn n => E n) pu_dt2
      in
        P.data (dt1ToInt, [pu_D, pu_E])
      end

  val newPu_dt2 : pu_dt2 P.pu =
      let
        fun dt2ToInt (F _) = 0
        fun pu_F pu_dt2 = P.con1 (fn (F dt1) => dt1) (fn dt1 => F dt1) pu_dt1
      in
