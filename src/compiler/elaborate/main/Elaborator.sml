...
*)
  ...
  fun replaceTyVarInTyWithTy (tyVars, argTys) ty =
      let
        ...
        fun subst (tyID as ({name, ifeq}, loc)) =
...
*)
  ...
  and elabExp env ast = 
      ...
      | A.EXPIF (e1, e2, e3, loc) =>
        ...
        in
          PC.PLCASEM
          ([ple1], [([truePat loc], ple2), ([falsePat loc], ple3)], Types.MATCH, loc)
        end
        ...
        | A.DECTYPE (tyBinds, loc) =>
          let
            fun elabTyBind (tvars, name, ty) =
                let
                  val newTVars =
                      map (fn {name=tvarName, ifeq} => {name=tvarName, ifeq=false}) tvars
                  val newTy =
                      substTyVarInTy
                          (fn ({name=tvarName, ifeq}, loc) =>
                              A.TYID({name=tvarName, ifeq=false}, loc))
                          ty
                in
...
(*******************below dealing with module*********************************)
    ...
    fun elaborate env decs =
        let        
          ...
          val (ptopdecls, env) = elabTopDecs env decs
        in        
          case getErrors () of
            [] => (ptopdecls, env, getWarnings())
