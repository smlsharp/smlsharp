...
struct
  type ord_key = {path:Longsymbol.longsymbol option, loc:Loc.loc, id:VarID.id, ty:Types.ty, opaque:bool}
  fun compare ({path = _, loc = _, ty = _, opaque = _, id = varId1} : ord_key,
	       {path = _, loc = _, ty = _, opaque = _, id = varId2} : ord_key) =
