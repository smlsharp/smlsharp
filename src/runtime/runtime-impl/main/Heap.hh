...
*/
{
  ...
  public:
    ...
    /** single atom array type */
    static const UInt32Value BLOCKTYPE_SINGLE_ATOM_ARRAY = 4;
    /** double atom array type */
    static const UInt32Value BLOCKTYPE_DOUBLE_ATOM_ARRAY = 5;
    /** pointer array type */
    static const UInt32Value BLOCKTYPE_POINTER_ARRAY = 6;
  ...
  private:
     ...
     *
     */
    static const UInt32Value SIZE_MASK =  0xFFFFFFFUL;
    static const UInt32Value SIZE_SHIFT = 0;
    static const UInt32Value TYPE_MASK = 0x70000000UL;
    static const UInt32Value TYPE_SHIFT = 28;
...
//        INVOKE_ON_HEAP_MONITORS(afterUpdateField(block, index, value));
        ...
        throw(IMLRuntimeException)
    {
        ...
        BlockHeader* header = allocRawBlock(fields);
        setHeader
        (header, fields, BLOCKTYPE_SINGLE_ATOM_ARRAY, BOOLVALUE_FALSE);
        Cell* block = HEADER_TO_BLOCK(header);
        ASSERT(isAlignedBlockAddress(block));
        ...
        throw(IMLRuntimeException)
    {
        ...
        BlockHeader* header = allocRawBlock(fields);
        setHeader
        (header, fields, BLOCKTYPE_DOUBLE_ATOM_ARRAY, BOOLVALUE_FALSE);
        Cell* block = HEADER_TO_BLOCK(header);
        ASSERT(isAlignedBlockAddress(block));
        INVOKE_ON_HEAP_MONITORS(afterAllocAtomArray(fields, block));
        return block;
        ...
        throw(IMLRuntimeException)
    {
        ...
        BlockHeader* header = allocRawBlock(fields);
        setHeader(header, fields, BLOCKTYPE_POINTER_ARRAY, BOOLVALUE_FALSE);
        Cell* block = HEADER_TO_BLOCK(header);
        ASSERT(isAlignedBlockAddress(block));
